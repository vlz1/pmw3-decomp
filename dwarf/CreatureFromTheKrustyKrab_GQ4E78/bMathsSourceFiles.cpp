/*
    Compile unit: b:/BlitzSDK/Babel/GameCube/Src/bMaths/bMathsSourceFiles.cpp
    Producer: GNU C++ 2.95.3 SN BUILD v1.62 for Nintendo Gamecube
    Compile directory: b:\BlitzSDK\Babel\GameCube\Src\bMaths
    Language: C++
    Code range: 0x8024FFF4 -> 0x8025756C
    GCC Source File Name Offset: 0x0001B7A6
    GCC Source Info Offset: 0x0004E4B0
*/
...
// Range: 0xFFFFFFFF -> 0x8024FFF4
int bmGeomWindingOrder2D(const float * point0 /* r3 */, const float * point1 /* r4 */, const float * point2 /* r5 */) {
    // Local variables
    float v1[2]; // r1+0x8
    float v2[2]; // r1+0x10
    float slope1; // f13
...
// Range: 0xFFFFFFFF -> 0x8024FFF4
float bmGeomDistanceFromLine2D(const struct _TBCollisionLine2D * const line /* r29 */, const float * point /* r28 */, float * closestPoint /* r30 */) {
    // Range: 0x8024FFF4 -> 0x8024FFF4
    inline float bmGeomGetClosestPointOnLineSegment2D(const float * lineStartPoint, const float * lineEndPoint, const float * point, float * closestPoint) {
        // Local variables
        float ret;
        float pointOffset[2]; // r1+0x10
        float lineDirection[2]; // r1+0x8
...
// Range: 0x8024FFF4 -> 0x802500C4
float bmGeomDistanceFromLine(const struct _TBCollisionLine * const line /* r29 */, const float * point /* r28 */, float * closestPoint /* r30 */) {
    // Range: 0x80250004 -> 0x80250004
    inline float bmGeomGetClosestPointOnLineSegment(const float * lineStartPoint, const float * lineEndPoint, const float * point, float * closestPoint) {
        // Local variables
        float ret;
        float pointOffset[4]; // r1+0x18
        float lineDirection[4]; // r1+0x8
...
// Range: 0x802500C4 -> 0x80250144
float bmGeomDistanceFromInfiniteLine(const struct _TBCollisionLine * const line /* r3 */, const float * point /* r29 */, float * closestPoint /* r30 */) {
    // Local variables
    float lineVector[4]; // r1+0x8

    // Range: 0x802500D4 -> 0x802500D4
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
}

// Range: 0xFFFFFFFF -> 0x80250144
void bmGeomGetClosestPointOnInfiniteOriginLine(const float * lineDirection /* r3 */, const float * point /* r4 */, float * closestPoint /* r5 */) {
    // Local variables
    float distanceMultipleAlongLine; // f12

    // Range: 0x80250144 -> 0x80250144
    inline float bmVanillaVectorLengthAlongVector(const float * basis, const float * test) {
        // Local variables
        float basisLenSquared; // f8
...
// Range: 0x80250144 -> 0x802501F8
void bmGeomGetClosestPointOnInfiniteLine(const float * linePoint /* r3 */, const float * lineDirection /* r4 */, const float * point /* r5 */, float * closestPoint /* r6 */) {
    // Local variables
    float pointOffset[4]; // r1+0x8
    float distanceMultipleAlongLine; // f10

    // Range: 0x80250148 -> 0x80250148
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}

    // Range: 0x80250148 -> 0x802501C0
    inline float bmVanillaVectorLengthAlongVector(const float * basis, const float * test) {
        // Local variables
        float basisLenSquared; // f6

        // Range: 0x80250148 -> 0x80250148
        inline float bmVanillaVectorSquaredLen(const float * src) {}

        // Range: 0x802501AC -> 0x802501AC
        inline float bmVanillaVectorDot(const float * src1, const float * src2) {}
    }

    // Range: 0x802501C0 -> 0x802501F0
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul, const float * vadd) {}
}

// Range: 0x802501F8 -> 0x802502B4
float bmGeomGetClosestPointOnOriginLineSegment(const float * lineEndPoint /* r3 */, const float * point /* r4 */, float * closestPoint /* r5 */) {
    // Local variables
    float distanceOffsetAlongLength; // f12
    float squaredLineLength;
    float fraction; // f12

    // Range: 0x802501F8 -> 0x802501F8
    inline float bmVanillaVectorDot(const float * src1, const float * src2) {}

    // Range: 0x8025022C -> 0x8025022C
    inline void bmVanillaVectorZero(float * dest) {}

    // Range: 0x8025024C -> 0x8025024C
    inline float bmVanillaVectorSquaredLen(const float * src) {}

    // Range: 0x80250260 -> 0x80250260
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025028C -> 0x8025028C
    inline void bmVanillaVectorCMul(float * dest, const float * src, const float val) {}
}

// Range: 0xFFFFFFFF -> 0x802502B4
float bmGeomGetClosestPointOnOriginLineSegment2D(const float * lineEndPoint /* r3 */, const float * point /* r4 */, float * closestPoint /* r5 */) {
    // Local variables
    float distanceOffsetAlongLength; // f13
    float squaredLineLength;
    float fraction; // f13

    // Range: 0x802502B4 -> 0x802502B4
    inline float bmVector2DDot(const float * source1, const float * source2) {}
...
// Range: 0xFFFFFFFF -> 0x802502B4
float bmGeomDistanceFromPlane(const float * plane /* r3 */, const float * point /* r4 */, float * closestPoint /* r5 */) {
    // Local variables
    float distFromPlane; // f1
...
// Range: 0xFFFFFFFF -> 0x802502B4
int bmGeomGetInfiniteLinePlaneIntersection(const float * plane /* r3 */, const float * linePoint /* r4 */, const float * lineDirection /* r5 */, float * intersection /* r6 */) {
    // Local variables
    float distFromPlane; // f12
    float speedTowardsPlane; // f7
    float time; // f12
...
// Range: 0x802502B4 -> 0x8025038C
int bmGeomLineLine2D(const struct _TBCollisionLine2D * const line1 /* r3 */, const struct _TBCollisionLine2D * const line2 /* r4 */) {
    // Local variables
    float line1Vec[2]; // r1+0x8
    float line2Vec[2]; // r1+0x10
    float relPos[2]; // r1+0x18
    float top1; // f13
    float top2; // f0
    float bot; // f10
    float bot2; // f6
...
// Range: 0x8025038C -> 0x802506D8
void bmSetMomentOfInertia(float (* moi)[4] /* r3 */, enum EBBodyType type /* r4 */, float mass /* f7 */, float w /* f2 */, float l /* f3 */, float h /* f13 */, int flags /* r5 */) {
    // Local variables
    float w2; // f2
    float l2; // f4
    float h2; // f13
    ...
    // Range: 0x80250680 -> 0x802506D4
    inline void bmSetMomentOfInertia(float (* moi)[4], float Ixx, float Iyy, float Izz, float Ixy, float Ixz, float Iyz) {}
}

// Range: 0x802506D8 -> 0x80250748
static void bmGetOffsetMomentOfInertia(float * Ixx /* r3 */, float * Iyy /* r4 */, float * Izz /* r5 */, float * Ixy /* r6 */, float * Ixz /* r7 */, float * Iyz /* r8 */, const float * offset /* r9 */, float mass /* f1 */) {
    // Local variables
    float x; // f13
    float y; // f10
    float z; // f9
...
// Range: 0x80250748 -> 0x8025083C
void bmAddMomentOfInertia(float (* moi)[4] /* r30 */, const float * offset /* r9 */, float (* momentOfInertia)[4] /* r29 */, float mass /* f1 */) {
    // Local variables
    float Ixx; // r1+0x8
    float Iyy; // r1+0xC
    float Izz; // r1+0x10
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmSubtractMomentOfInertia(float (* moi)[4] /* r30 */, const float * offset /* r9 */, float (* momentOfInertia)[4] /* r29 */, float mass /* f1 */) {
    // Local variables
    float Ixx; // r1+0x8
    float Iyy; // r1+0xC
    float Izz; // r1+0x10
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmShiftMomentOfInertia(float (* moi)[4] /* r31 */, const float * offset /* r9 */, float mass /* f1 */, int flags /* r30 */) {
    // Local variables
    float Ixx; // r1+0x8
    float Iyy; // r1+0xC
    float Izz; // r1+0x10
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmForceOfSpring3D(float * result /* r29 */, float kS /* f31 */, float r /* f30 */, const float * pos1 /* r4 */, const float * pos2 /* r5 */) {
    // Local variables
    float L[4]; // r1+0x8
    float Ln[4]; // r1+0x18
    float l;

    // Range: 0x8025083C -> 0x8025083C
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmForceOfDampedSpring3D(float * result /* r27 */, float kS /* f29 */, float r /* f31 */, float kD /* f30 */, const float * pos1 /* r4 */, const float * pos2 /* r5 */, const float * v1 /* r29 */, const float * v2 /* r30 */) {
    // Local variables
    float L[4]; // r1+0x8
    float v[4]; // r1+0x18
    float Ln[4]; // r1+0x28
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmSetMomentOfInertia2D(float * moi /* r3 */, enum EBBodyType2D type /* r4 */, float mass /* f1 */, float w /* f2 */, float l /* f3 */, float h /* f4 */) {}

// Range: 0xFFFFFFFF -> 0x8025083C
void bmAddMomentOfInertia2D(float * moi /* r30 */, const float * offset /* r4 */, float momentOfInertia /* f30 */, float mass /* f31 */) {
    // Local variables
    float r;
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmAddMomentOfInertia2D(float * moi /* r3 */, float r /* f1 */, float momentOfInertia /* f2 */, float mass /* f3 */) {}

// Range: 0xFFFFFFFF -> 0x8025083C
void bmSubtractMomentOfInertia2D(float * moi /* r30 */, const float * offset /* r4 */, float momentOfInertia /* f30 */, float mass /* f31 */) {
    // Local variables
    float r;
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmSubtractMomentOfInertia2D(float * moi /* r3 */, float r /* f1 */, float momentOfInertia /* f2 */, float mass /* f3 */) {}

// Range: 0xFFFFFFFF -> 0x8025083C
void bmShiftMomentOfInertia2D(float * moi /* r31 */, const float * offset /* r4 */, float mass /* f31 */, int flags /* r30 */) {
    // Local variables
    float r;
...
// Range: 0xFFFFFFFF -> 0x8025083C
void bmShiftMomentOfInertia2D(float * moi /* r3 */, float r /* f1 */, float mass /* f2 */, int flags /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025083C
void bmCalculateRelativeVelocity2D(float * rv /* r3 */, const float * v /* r4 */, float w /* f1 */, const float * r /* r5 */) {
    // Local variables
    float rT[4]; // r1+0x8
    float wv[4]; // r1+0x18
...
// Range: 0xFFFFFFFF -> 0x8025083C
int bmVector2DWindingOrder(const float * point0 /* r3 */, const float * point1 /* r4 */, const float * point2 /* r5 */) {
    // Local variables
    float v1[2]; // r1+0x8
    float v2[2]; // r1+0x10
    float slope1; // f13
...
// Range: 0xFFFFFFFF -> 0x8025083C
float bmVector2DDistanceFromLineSection(const float * start /* r30 */, const float * end /* r27 */, const float * point /* r29 */, float * closestPoint /* r31 */) {
    // Local variables
    float startPointVector[2]; // r1+0x18
    float unitLineVector[2]; // r1+0x8
    float pointDistance; // f1
    float startPointDistance; // f11
...
// Range: 0xFFFFFFFF -> 0x8025083C
float bmVector2DDistanceFromInfiniteLine(const float * start /* r31 */, const float * end /* r4 */, const float * point /* r30 */, float * closestPoint /* r29 */) {
    // Local variables
    float startPointVector[2]; // r1+0x18
    float unitLineVector[2]; // r1+0x8
    float pointDistance; // f1
    float startPointDistance; // f12
    float startEndDistance;
...
// Range: 0xFFFFFFFF -> 0x8025083C
int bmVector2DIntersectInfiniteLines(const float * start1 /* r3 */, const float * end1 /* r4 */, const float * start2 /* r5 */, const float * end2 /* r6 */, float * const t /* r7 */, float * const s /* r8 */, float * point /* r9 */) {
    // Local variables
    float vector1[2]; // r1+0x8
    float vector2[2]; // r1+0x10
    float num; // f0
    float det; // f9
    float temp; // f11
...
// Range: 0xFFFFFFFF -> 0x8025083C
int bmVector2DTestLineSegments(const float * start1 /* r3 */, const float * end1 /* r4 */, const float * start2 /* r5 */, const float * end2 /* r6 */) {
    // Local variables
    float x4_x3; // f13
    float y4_y3; // f12
    float x2_x1; // f7
...
// Range: 0x8025083C -> 0x80250888
float bmVectorXZDistance(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80250848 -> 0x80250848
    inline void bmVectorXZSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80250888 -> 0x802508C0
float bmVectorXZSquaredDistance(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x8025088C -> 0x8025088C
    inline void bmVectorXZSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0xFFFFFFFF -> 0x802508C0
float bmVectorXZScaleToLength(float * dest /* r30 */, const float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f0

    // Range: 0x802508C0 -> 0x802508C0
    inline float bmVectorXZLen(const float * src) {
        // Range: 0x802508C0 -> 0x802508C0
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x802508C0 -> 0x80250934
float bmVectorXZScaleToLength(float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f12

    // Range: 0x802508D4 -> 0x802508D4
    inline float bmVectorXZLen(const float * src) {
        // Range: 0x802508D4 -> 0x802508D4
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x80250934 -> 0x802509C4
float bmVectorXZNorm(float * dest /* r30 */, const float * src /* r31 */) {
    // Local variables
    float length;
    float oneOverLength; // f13

    // Range: 0x80250944 -> 0x80250944
    inline float bmVectorXZLen(const float * src) {
        // Range: 0x80250944 -> 0x80250944
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x802509C4 -> 0x802509FC
void bmVectorXZLerp(float * dest /* r3 */, const float * src1 /* r4 */, const float * src2 /* r5 */, const float t /* f1 */) {
    // Local variables
    float oneMinusT; // f12
}

// Range: 0x802509FC -> 0x80250A68
float bmVectorXZDistanceApprox(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80250A00 -> 0x80250A00
    inline void bmVectorXZSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80250A68 -> 0x80250B04
struct TBDataArray * bmDataArrayCreate(int maxElements /* r29 */, int flags /* r30 */) {
    // Local variables
    struct TBDataArray * data; // r11
    char * ptr; // r0
    int size1;
    int size2; // r31
    int totalSize; // r3
...
// Range: 0x80250B04 -> 0x80250BA0
struct TBDataArrayWide * bmDataArrayCreateWide(int maxElements /* r29 */, int flags /* r30 */) {
    // Local variables
    struct TBDataArrayWide * data; // r11
    char * ptr; // r0
    int size1;
    int size2; // r31
    int totalSize; // r3
...
// Range: 0xFFFFFFFF -> 0x80250BA0
struct TBDataArrayWide * bmDataArrayCopy(struct TBDataArrayWide * dest /* r31 */, struct TBDataArrayWide * src /* r30 */) {
    // Local variables
    int size; // r3
    int arraySize; // r29
    char * space; // r9
...
// Range: 0xFFFFFFFF -> 0x80250BA0
struct TBDataArray * bmDataArrayCopy(struct TBDataArray * dest /* r31 */, struct TBDataArray * src /* r30 */) {
    // Local variables
    int size; // r3
    int arraySize; // r29
    char * space; // r9
...
// Range: 0xFFFFFFFF -> 0x80250BA0
int bDataArrayCompareElement(const void * el1 /* r3 */, const void * el2 /* r4 */) {
    // Local variables
    struct TBDataElement * e1;
    struct TBDataElement * e2;
}

// Range: 0xFFFFFFFF -> 0x80250BA0
int bDataArrayCompareElementWide(const void * el1 /* r3 */, const void * el2 /* r4 */) {
    // Local variables
    struct TBDataElementWide * e1;
    struct TBDataElementWide * e2;
}

// Range: 0xFFFFFFFF -> 0x80250BA0
void bDataArraySortBubble(struct TBDataArray * data /* r3 */) {
    // Local variables
    int low; // r7
    int last; // r6
    int high; // r8
    int i; // r10
    int j; // r5
    struct TBDataElement * array; // r4

    // Range: 0x80250BA0 -> 0x80250BA0
    inline void bmDataArraySwapElements(struct TBDataElement * e1, struct TBDataElement * e2) {
        // Local variables
        void * context; // r9
        float value;
...
// Range: 0xFFFFFFFF -> 0x80250BA0
void bDataArraySortBubble(struct TBDataArrayWide * data /* r3 */) {
    // Local variables
    int low; // r12
    int last; // r3
    int high; // r4
    int i; // r7
    int j; // r31
    struct TBDataElementWide * array; // r30
...
// Range: 0x80250BA0 -> 0x80250DA8
void bDataArrayPartition(struct TBDataElement * firstIn /* r7 */, struct TBDataElement * lastIn /* r30 */, float partition /* f1 */) {
    // Local variables
    struct TBDataElement * first; // r10
    struct TBDataElement * last; // r3
    struct TBDataElement * middle; // r11
    struct TBDataElement * boundaryLower; // r4
    struct TBDataElement * boundaryUpper; // r31
    int found; // r8
    int compare; // r0
    struct TBDataElement partitionElement; // r1+0x8

    // Range: 0x80250C2C -> 0x80250C2C
    inline void bmDataArraySwapElements(struct TBDataElement * e1, struct TBDataElement * e2) {
        // Local variables
        void * context; // r9
        float value;
...
// Range: 0xFFFFFFFF -> 0x80250DA8
void bDataArrayPartition(struct TBDataElementWide * firstIn /* r12 */, struct TBDataElementWide * lastIn /* r30 */, float partitionValue /* f1 */, int partitionPriority /* r5 */) {
    // Local variables
    struct TBDataElementWide * first; // r7
    struct TBDataElementWide * last; // r3
    struct TBDataElementWide * middle; // r8
    struct TBDataElementWide * boundaryLower; // r4
    struct TBDataElementWide * boundaryUpper; // r31
    int found; // r4
    int compare; // r0
    struct TBDataElementWide partitionElement; // r1+0x8

    // Range: 0x80250DA8 -> 0x80250DA8
    inline void bmDataArraySwapElements(struct TBDataElementWide * e1, struct TBDataElementWide * e2) {
        // Local variables
        int priority;
        void * context2; // r11
...
// Range: 0x80250DA8 -> 0x80250EFC
void bDataArraySortMerge(struct TBDataArray * data /* r25 */) {
    // Local variables
    struct TBDataElement * from; // r31
    struct TBDataElement * to; // r28
    struct TBDataElement * merge1; // r11
    struct TBDataElement * merge2; // r10
    struct TBDataElement * target; // r9
    int chunkSize; // r5
    int chunkSize1; // r7
    int chunkSize2; // r8
    int i; // r3
    ...
    // Range: 0x80250E4C -> 0x80250E4C
    inline void bmDataArrayCopyElement(struct TBDataElement * dest, struct TBDataElement * src) {}

    // Range: 0x80250E68 -> 0x80250E68
    inline void bmDataArrayCopyElement(struct TBDataElement * dest, struct TBDataElement * src) {}

    // Range: 0x80250E84 -> 0x80250E84
    inline void bmDataArrayCopyElement(struct TBDataElement * dest, struct TBDataElement * src) {}

    // Range: 0x80250E98 -> 0x80250E98
    inline void bmDataArrayCopyElement(struct TBDataElement * dest, struct TBDataElement * src) {}
}

// Range: 0x80250EFC -> 0x80251098
void bDataArraySortMerge(struct TBDataArrayWide * data /* r26 */) {
    // Local variables
    struct TBDataElementWide * from; // r30
    struct TBDataElementWide * to; // r28
    struct TBDataElementWide * merge1; // r8
    struct TBDataElementWide * merge2; // r10
    struct TBDataElementWide * target; // r11
    int chunkSize; // r4
    int chunkSize1; // r6
    int chunkSize2; // r7
    int i; // r31
    ...
    // Range: 0x80250FA0 -> 0x80250FA0
    inline void bmDataArrayCopyElement(struct TBDataElementWide * dest, struct TBDataElementWide * src) {}

    // Range: 0x80250FCC -> 0x80250FCC
    inline void bmDataArrayCopyElement(struct TBDataElementWide * dest, struct TBDataElementWide * src) {}

    // Range: 0x80250FFC -> 0x80250FFC
    inline void bmDataArrayCopyElement(struct TBDataElementWide * dest, struct TBDataElementWide * src) {}

    // Range: 0x80251024 -> 0x80251024
    inline void bmDataArrayCopyElement(struct TBDataElementWide * dest, struct TBDataElementWide * src) {}
}

// Range: 0x80251098 -> 0x80251144
float bmVanillaSinApproxMinusPiToPi(float x /* f1 */) {
    // Local variables
    bool test; // r0
    float x2; // f12
    float y; // f0
...
// Range: 0x80251144 -> 0x80251180
float bmVanillaSinApprox0To2Pi(float x /* f1 */) {}

// Range: 0x80251180 -> 0x802511CC
float bmVanillaSinApprox(float x /* f31 */) {}

// Range: 0x802511CC -> 0x8025128C
float bmVanillaCosApproxMinusPiToPi(float x /* f1 */) {
    // Local variables
    bool test; // r0
    float x2; // f11
    float y; // f0
...
// Range: 0x8025128C -> 0x802512C8
float bmVanillaCosApprox0To2Pi(float x /* f1 */) {}

// Range: 0x802512C8 -> 0x80251314
float bmVanillaCosApprox(float x /* f31 */) {}

// Range: 0x80251314 -> 0x80251398
float bmVanillaAngularLimit(float angle /* f1 */) {
    // Local variables
    int num; // r0
}

// Range: 0x80251398 -> 0x802513FC
float bmVanillaFindShortestAngle(float ang1 /* f12 */, float ang2 /* f2 */) {
    // Local variables
    float diff; // f13
    float absDiff;

    // Range: 0x8025139C -> 0x8025139C
    inline float bmVanillaFAbs(float x) {}
}

// Range: 0x802513FC -> 0x8025145C
float bmVanillaFindShortestAngleSigned(float ang1 /* f1 */, float ang2 /* f2 */) {
    // Local variables
    float diff; // f2
}

// Range: 0x8025145C -> 0x80251520
float bmVanillaAngleSlerp(const float src /* f31 */, const float dest /* f2 */, float t /* f30 */) {
    // Local variables
    float diff;

    // Range: 0x80251470 -> 0x80251508
    inline float bmVanillaFMod(float x, float y) {
        // Local variables
        float v; // f0
        float py;
...
// Range: 0xFFFFFFFF -> 0x80251520
float bmVanillaFactorial(int n /* r31 */) {}

// Range: 0x80251520 -> 0x80251864
void bmVanillaMatMultiplyUnaligned44(float (* dest)[4] /* r3 */, float (* src1)[4] /* r4 */, float (* src2)[4] /* r5 */) {}

// Range: 0xFFFFFFFF -> 0x80251864
void bmVanillaMatMultiplyUnaligned44(float (* src1)[4] /* r3 */, float (* src2)[4] /* r4 */) {
    // Local variables
    float tmp[4][4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80251864
void bmVanillaMatMultiply44(float (* src1)[4] /* r3 */, float (* src2)[4] /* r4 */) {
    // Local variables
    float tmp[4][4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80251864
char * bmVanillaMatShow(float (* mat)[4] /* r3 */, char * const buf /* r30 */) {
    // Local variables
    int row;
}

// Range: 0x80251864 -> 0x802518FC
void bmVanillaMatXRotation(float (* mat)[4] /* r30 */, const float radians /* f31 */) {
    // Range: 0x80251878 -> 0x80251878
    inline float bmVanillaCos(float val) {}
...
// Range: 0x802518FC -> 0x80251994
void bmVanillaMatYRotation(float (* mat)[4] /* r30 */, const float radians /* f31 */) {
    // Range: 0x80251910 -> 0x80251910
    inline float bmVanillaCos(float val) {}
...
// Range: 0x80251994 -> 0x80251A2C
void bmVanillaMatZRotation(float (* mat)[4] /* r30 */, const float radians /* f31 */) {
    // Range: 0x802519A8 -> 0x802519A8
    inline float bmVanillaCos(float val) {}
...
// Range: 0x80251A2C -> 0x80251A80
void bmVanillaMatTranslate(float (* mat)[4] /* r3 */, const float x /* f1 */, const float y /* f2 */, const float z /* f3 */) {}

// Range: 0x80251A80 -> 0x80251AD4
void bmVanillaMatScale(float (* mat)[4] /* r3 */, const float x /* f1 */, const float y /* f2 */, const float z /* f3 */) {}

// Range: 0xFFFFFFFF -> 0x80251AD4
void bmVanillaMatMultiplyVector(float (* mat)[4] /* r3 */, float * vec /* r4 */) {
    // Local variables
    float x; // f8
    float y; // f9
    float z; // f10
...
// Range: 0x80251AD4 -> 0x80251B78
void bmVanillaMatMultiplyVectorWithW(float (* mat)[4] /* r3 */, float * vec /* r4 */) {
    // Local variables
    float x; // f7
    float y; // f8
    float z; // f9
...
// Range: 0x80251B78 -> 0x80251C18
void bmVanillaMatMultiplyVector2(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {
    // Local variables
    float x; // f9
    float y; // f8
    float z; // f10
...
// Range: 0xFFFFFFFF -> 0x80251C18
void bmVanillaMatMultiplyVector2WithW(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {
    // Local variables
    float x; // f8
    float y; // f7
    float z; // f9
...
// Range: 0xFFFFFFFF -> 0x80251C18
void bmVanillaMatMultiply33Vector(float (* mat)[4] /* r3 */, float * vec /* r4 */) {
    // Local variables
    float x; // f9
    float y; // f10
    float z; // f11
...
// Range: 0x80251C18 -> 0x80251C7C
void bmVanillaMatMultiply33Vector2(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {
    // Local variables
    float x; // f10
    float y; // f9
    float z; // f11
...
// Range: 0xFFFFFFFF -> 0x80251C7C
void bmVanillaMatTranspose(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x80251C7C
void bmVanillaMatTranspose(float (* mat)[4] /* r3 */) {
    // Local variables
    float tmp[4][4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80251C7C
void bmVanillaMatCombineXRotation(float (* mat)[4] /* r31 */, const float radians /* f31 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48

    // Range: 0x80251C7C -> 0x80251C7C
    inline float bmVanillaCos(float val) {}
...
// Range: 0x80251C7C -> 0x80251FC8
void bmVanillaMatCombineYRotation(float (* mat)[4] /* r31 */, const float radians /* f31 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48

    // Range: 0x80251CD4 -> 0x80251CD4
    inline float bmVanillaCos(float val) {}
...
// Range: 0x80251FC8 -> 0x80252314
void bmVanillaMatCombineZRotation(float (* mat)[4] /* r31 */, const float radians /* f31 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48

    // Range: 0x80252020 -> 0x80252020
    inline float bmVanillaCos(float val) {}
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineTranslate(float (* mat)[4] /* r3 */, const float x /* f1 */, const float y /* f2 */, const float z /* f3 */) {
    // Local variables
    float trans[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
}

// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineScale(float (* mat)[4] /* r3 */, const float x /* f1 */, const float y /* f2 */, const float z /* f3 */) {
    // Local variables
    float scale[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
}

// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineXYRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f29 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineYXRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f29 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineXZRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float z /* f29 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineZXRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float z /* f30 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineYZRotation(float (* mat)[4] /* r31 */, const float y /* f31 */, const float z /* f29 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float yCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineZYRotation(float (* mat)[4] /* r31 */, const float y /* f31 */, const float z /* f29 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float yCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineXYZRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f27 */, const float z /* f26 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineXZYRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f30 */, const float z /* f26 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineYXZRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f30 */, const float z /* f26 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineYZXRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f29 */, const float z /* f26 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineZXYRotation(float (* mat)[4] /* r31 */, const float x /* f31 */, const float y /* f30 */, const float z /* f27 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatCombineZYXRotation(float (* mat)[4] /* r31 */, const float x /* f30 */, const float y /* f29 */, const float z /* f26 */) {
    // Local variables
    float rot[4][4]; // r1+0x8
    float tmp[4][4]; // r1+0x48
    float xCos;
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatMultiply33(float (* dest)[4] /* r3 */, float (* src1)[4] /* r4 */, float (* src2)[4] /* r5 */) {}

// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatTransMultiply33Vector2(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {
    // Local variables
    float x; // f10
    float y; // f9
    float z; // f11
...
// Range: 0xFFFFFFFF -> 0x80252314
void bmVanillaMatTransMultiply33Vector(float (* mat)[4] /* r3 */, float * vec /* r4 */) {
    // Local variables
    float x; // f9
    float y; // f10
    float z; // f11
...
// Range: 0x80252314 -> 0x80252458
int bmVanillaMatFactorXYZ(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float y; // f31

    // Range: 0x80252374 -> 0x8025237C
    inline float bmVanillaASin(float val) {}

    // Range: 0x802523A4 -> 0x802523A4
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x802523CC -> 0x802523CC
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x802523F0 -> 0x802523F0
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x8025240C -> 0x8025240C
    inline float bmVanillaATan2(float y, float x) {}
}

// Range: 0xFFFFFFFF -> 0x80252458
int bmVanillaMatFactorXZY(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float z; // f31

    // Range: 0x80252458 -> 0x80252458
    inline float bmVanillaASin(float val) {}
...
// Range: 0x80252458 -> 0x802525A0
int bmVanillaMatFactorYXZ(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float x; // f1

    // Range: 0x802524B8 -> 0x802524BC
    inline float bmVanillaASin(float val) {}

    // Range: 0x802524F0 -> 0x802524F0
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80252508 -> 0x80252508
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80252534 -> 0x80252534
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80252560 -> 0x80252560
    inline float bmVanillaATan2(float y, float x) {}
}

// Range: 0x802525A0 -> 0x802526F8
int bmVanillaMatFactorYZX(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float z; // f31

    // Range: 0x80252600 -> 0x80252608
    inline float bmVanillaASin(float val) {}

    // Range: 0x80252630 -> 0x80252630
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x8025264C -> 0x8025264C
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80252690 -> 0x80252690
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x802526C0 -> 0x802526C0
    inline float bmVanillaATan2(float y, float x) {}
}

// Range: 0x802526F8 -> 0x80252850
int bmVanillaMatFactorZXY(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float x; // f1

    // Range: 0x80252754 -> 0x80252758
    inline float bmVanillaASin(float val) {}

    // Range: 0x8025278C -> 0x8025278C
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x802527A8 -> 0x802527A8
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x802527EC -> 0x802527EC
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80252828 -> 0x80252828
    inline float bmVanillaATan2(float y, float x) {}
}

// Range: 0xFFFFFFFF -> 0x80252850
int bmVanillaMatFactorZYX(float (* mat)[4] /* r31 */, float * xRot /* r30 */, float * yRot /* r29 */, float * zRot /* r28 */) {
    // Local variables
    float y; // f31

    // Range: 0x80252850 -> 0x80252850
    inline float bmVanillaASin(float val) {}
...
// Range: 0x80252850 -> 0x80252938
void bmVanillaMatrix33Solve(float (* A)[4] /* r3 */, float * x /* r4 */, const float * b /* r5 */) {
    // Local variables
    int i; // ctr
    float tmp; // f10
    float inv[4]; // r1+0x8
...
static inline void MatrixCofactor(float (* cofactor)[4], float (* src)[4], int i, int j) {
    // Local variables
    int r1;
    int r2;
    int c1;
...
// Range: 0xFFFFFFFF -> 0x80252938
void bmVanillaMatInverseFull(float (* dest)[4] /* r31 */, float (* src)[4] /* r26 */) {
    // Local variables
    float temp[4][4]; // r1+0x8
    float cofactor[4][4]; // r1+0x48
    int i; // r6
    int j; // r3
    int c; // r30
    float determinant;

    // Range: 0x80252938 -> 0x80252938
    inline void MatrixCofactor(float (* cofactor)[4], float (* src)[4], int i, int j) {
        // Local variables
        int c2;
        int c1; // r10
        int r2; // r8
...
// Range: 0xFFFFFFFF -> 0x80252938
float bmVanillaMatDet(float (* mat)[4] /* r3 */) {
    // Local variables
    float det; // f1
}

// Range: 0x80252938 -> 0x80252A18
void bmVanillaMatXYRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f30 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80252A18 -> 0x80252AF8
void bmVanillaMatYXRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f29 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0xFFFFFFFF -> 0x80252AF8
void bmVanillaMatXZRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float z /* f29 */) {
    // Local variables
    float xCos;
    float xSin;
    float zCos;
...
// Range: 0xFFFFFFFF -> 0x80252AF8
void bmVanillaMatZXRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float z /* f30 */) {
    // Local variables
    float xCos;
    float xSin;
    float zCos;
...
// Range: 0xFFFFFFFF -> 0x80252AF8
void bmVanillaMatYZRotation(float (* dest)[4] /* r30 */, const float y /* f31 */, const float z /* f29 */) {
    // Local variables
    float yCos;
    float ySin;
    float zCos;
...
// Range: 0xFFFFFFFF -> 0x80252AF8
void bmVanillaMatZYRotation(float (* dest)[4] /* r30 */, const float y /* f31 */, const float z /* f29 */) {
    // Local variables
    float yCos;
    float ySin;
    float zCos;
...
// Range: 0x80252AF8 -> 0x80252C30
void bmVanillaMatXYZRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f27 */, const float z /* f26 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80252C30 -> 0x80252D68
void bmVanillaMatXZYRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f30 */, const float z /* f26 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80252D68 -> 0x80252EA0
void bmVanillaMatYXZRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f30 */, const float z /* f26 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80252EA0 -> 0x80252FD8
void bmVanillaMatYZXRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f29 */, const float z /* f26 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80252FD8 -> 0x80253110
void bmVanillaMatZXYRotation(float (* dest)[4] /* r30 */, const float x /* f31 */, const float y /* f30 */, const float z /* f27 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80253110 -> 0x80253248
void bmVanillaMatZYXRotation(float (* dest)[4] /* r30 */, const float x /* f30 */, const float y /* f29 */, const float z /* f26 */) {
    // Local variables
    float xCos;
    float xSin;
    float yCos;
...
// Range: 0x80253248 -> 0x802532D8
void bmVanillaMatTransOr(float (* dest)[4] /* r3 */, float xTrans /* f1 */, float yTrans /* f2 */, float zTrans /* f3 */, float (* orientation)[4] /* r4 */) {}

// Range: 0x802532D8 -> 0x80253374
void bmVanillaMatTransOrScale(float (* dest)[4] /* r3 */, float xTrans /* f1 */, float yTrans /* f2 */, float zTrans /* f3 */, float (* orientation)[4] /* r4 */, float xScale /* f4 */, float yScale /* f5 */, float zScale /* f6 */) {}

// Range: 0x80253374 -> 0x802533E0
void bmVanillaMatTransScale(float (* dest)[4] /* r3 */, const float * trans /* r4 */, const float * scale /* r5 */) {}

// Range: 0x802533E0 -> 0x802534C4
void bmVanillaMatPreApplyScale(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */, const float * scale /* r5 */) {}

// Range: 0x802534C4 -> 0x802535A8
void bmVanillaMatPostApplyScale(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */, const float * scale /* r5 */) {}

// Range: 0x802535A8 -> 0x80253694
void bmVanillaMatPostApplyTrans(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */, const float * trans /* r5 */) {
    // Range: 0x802535A8 -> 0x802535A8
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul, const float * vadd) {}
    ...
    // Range: 0x802535A8 -> 0x80253690
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul, const float * vadd) {}
}

// Range: 0xFFFFFFFF -> 0x80253694
void bmVanillaMatPostApplyTransScale(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */, const float * scaleIn /* r5 */, const float * transIn /* r6 */) {
    // Range: 0x80253694 -> 0x80253694
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul) {}
...
// Range: 0x80253694 -> 0x80253920
int bmVanillaMatInverse(float (* dest)[4] /* r11 */, float (* in)[4] /* r4 */) {
    // Local variables
    float det_1; // f2
    float pos; // f10
    float neg; // f12
    ...
    // Range: 0x802537E0 -> 0x802537E0
    inline float bmVanillaFAbs(float x) {}
}

// Range: 0x80253920 -> 0x80253AB4
void bmVanillaMatInverseNonSingular(float (* dest)[4] /* r3 */, float (* in)[4] /* r4 */) {
    // Local variables
    float det_1; // f11
    float det; // f0
    float in00; // f4
...
// Range: 0x80253AB4 -> 0x80253D10
void bmVanillaMatDecompose(float (* mat)[4] /* r3 */, float * pos /* r4 */, float * scale /* r31 */, float * orientation /* r30 */) {
    // Local variables
    float x0; // f25
    float y0; // f23
    float z0; // f24
    ...
    float yCross; // f12
    float zCross; // f0
    float omat[4][4]; // r1+0x8

    // Range: 0x80253AF0 -> 0x80253AF0
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x80253B78 -> 0x80253B78
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x80253BC0 -> 0x80253BC0
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0xFFFFFFFF -> 0x80253D10
void bmVanillaMatLookAt(float (* mat)[4] /* r30 */, const float * from /* r31 */, const float * at /* r5 */, float rollRadians /* f27 */) {
    // Local variables
    float xAng; // f1
    float yAng; // f29
    float dist;
...
// Range: 0x80253D10 -> 0x80253DB8
float bmVanillaVectorNorm(float * dest /* r30 */, const float * src /* r31 */) {
    // Local variables
    float length;
    float oneOverLength; // f12

    // Range: 0x80253D20 -> 0x80253D20
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x80253D20 -> 0x80253D20
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x80253DB8 -> 0x80253E88
float bmVanillaVectorNorm4(float * dest /* r30 */, const float * src /* r31 */) {
    // Local variables
    float length;
    float oneOverLength; // f12

    // Range: 0x80253DC8 -> 0x80253DC8
    inline float bmVanillaVectorLen4(const float * src) {
        // Range: 0x80253DC8 -> 0x80253DC8
        inline float bmVanillaSqrt(float x) {}
    }

    // Range: 0x80253E08 -> 0x80253E08
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0x80253E88 -> 0x80253EEC
float bmVanillaVectorDistance(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80253E94 -> 0x80253E94
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80253EEC -> 0x80253F70
float bmVanillaVectorDistanceApprox(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80253EF0 -> 0x80253EF0
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0xFFFFFFFF -> 0x80253F70
float bmVanillaVectorDistance4(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80253F70 -> 0x80253F70
    inline void bmVanillaVectorSub4(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80253F70 -> 0x80253FC0
float bmVanillaVectorSquaredDistance(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80253F74 -> 0x80253F74
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0xFFFFFFFF -> 0x80253FC0
float bmVanillaVectorSquaredDistance4(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float distance[4]; // r1+0x8

    // Range: 0x80253FC0 -> 0x80253FC0
    inline void bmVanillaVectorSub4(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80253FC0 -> 0x8025406C
float bmVanillaVectorScaleToLength(float * dest /* r30 */, const float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f12

    // Range: 0x80253FD4 -> 0x80253FD4
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x80253FD4 -> 0x80253FD4
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x8025406C -> 0x80254100
float bmVanillaVectorScaleToLength(float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f13

    // Range: 0x80254080 -> 0x80254080
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x80254080 -> 0x80254080
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0xFFFFFFFF -> 0x80254100
float bmVanillaVectorScaleToLength4(float * dest /* r30 */, const float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f12

    // Range: 0x80254100 -> 0x80254100
    inline float bmVanillaVectorLen4(const float * src) {
        // Range: 0x80254100 -> 0x80254100
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0xFFFFFFFF -> 0x80254100
float bmVanillaVectorScaleToLength4(float * src /* r31 */, const float length /* f31 */) {
    // Local variables
    float d;
    float len; // f13
...
// Range: 0x80254100 -> 0x8025414C
void bmVanillaVectorLerp(float * dest /* r3 */, const float * src1 /* r4 */, const float * src2 /* r5 */, const float t /* f1 */) {
    // Local variables
    float oneMinusT; // f12
}

// Range: 0x8025414C -> 0x802541AC
void bmVanillaVectorLerp4(float * dest /* r3 */, const float * src1 /* r4 */, const float * src2 /* r5 */, const float t /* f1 */) {
    // Local variables
    float oneMinusT; // f12
}

// Range: 0xFFFFFFFF -> 0x802541AC
int bmVanillaVectorLerpAtFixedSpeed(float * vectorToLerp /* r31 */, const float * target /* r4 */, float maxDistance /* f31 */) {
    // Local variables
    float offset[4]; // r1+0x8
    float squaredSeparation;

    // Range: 0x802541AC -> 0x802541AC
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x802541AC -> 0x802541EC
void bmVanillaVectorCombine(float * dest /* r3 */, const float * src1 /* r4 */, const float factor1 /* f1 */, const float * src2 /* r5 */, const float factor2 /* f2 */) {}

// Range: 0x802541EC -> 0x8025422C
void bmVanillaVectorCombine(float * src1 /* r3 */, const float factor1 /* f1 */, const float * src2 /* r4 */, const float factor2 /* f2 */) {}

// Range: 0x8025422C -> 0x80254280
void bmVanillaVectorCombine4(float * dest /* r3 */, const float * src1 /* r4 */, const float factor1 /* f1 */, const float * src2 /* r5 */, const float factor2 /* f2 */) {}

// Range: 0xFFFFFFFF -> 0x80254280
void bmVanillaVectorCombine4(float * src1 /* r3 */, const float factor1 /* f1 */, const float * src2 /* r4 */, const float factor2 /* f2 */) {}

// Range: 0xFFFFFFFF -> 0x80254280
void bmVanillaVectorCalcNormal(float * dest /* r31 */, const float * src1 /* r4 */, const float * src2 /* r5 */, const float * src3 /* r6 */) {
    // Local variables
    float ax; // f13
    float ay; // f1
    float az; // f12
    ...
    // Range: 0x80254280 -> 0x80254280
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0x80254280 -> 0x80254494
int bmVanillaVectorIntersectInfiniteLines(const float * start1 /* r3 */, const float * end1 /* r4 */, const float * start2 /* r5 */, const float * end2 /* r6 */, float * const d /* r31 */, float * const t /* r8 */, float * const s /* r7 */) {
    // Local variables
    float vector1[4]; // r1+0x8
    float vector2[4]; // r1+0x18
    float crossV1V2[4]; // r1+0x28
    float relPos[4]; // r1+0x38
    float squareMagnitude; // f1
    float m00; // f10
    float m01; // f12
    float m02; // f11
    float m10; // f12
    ...
    inline void bmVanillaVectorCross(float * dest, const float * src1, const float * src2) {
        // Local variables
        float tmpDest[4]; // r1+0x48
    }

    // Range: 0x802542A0 -> 0x802542A0
    inline float bmVanillaVectorSquaredLen(const float * src) {}

    // Range: 0x8025440C -> 0x8025440C
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80254494 -> 0x802546C8
float bmVanillaVectorDistanceFromLineSection(const float * start /* r30 */, const float * end /* r29 */, const float * point /* r28 */, float * closestPoint /* r31 */) {
    // Local variables
    float startPointVector[4]; // r1+0x8
    float unitLineVector[4]; // r1+0x18
    float pointDistance; // f12
    float startPointDistanceSqrd; // f1
    float startEndDistance;
    float pointDistanceSqrd; // f0

    // Range: 0x802544A4 -> 0x802544A4
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
    ...
    // Range: 0x80254510 -> 0x80254530
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x80254540 -> 0x80254540
    inline void bmVanillaVectorCDiv(float * dest, const float * src, const float val) {}
    ...
    // Range: 0x802545D0 -> 0x802545E8
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x802545E8 -> 0x802545E8
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x802545E8 -> 0x802545E8
        inline float bmVanillaSqrt(float x) {}
    }

    // Range: 0x802545F8 -> 0x802545F8
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}

    // Range: 0x80254624 -> 0x80254640
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x80254640 -> 0x80254640
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x80254640 -> 0x80254640
        inline float bmVanillaSqrt(float x) {}
    }

    // Range: 0x80254660 -> 0x80254660
    inline float bmVanillaVectorSquaredLen(const float * src) {}

    // Range: 0x80254674 -> 0x80254694
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul, const float * vadd) {}

    // Range: 0x802546B0 -> 0x802546B0
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0x802546C8 -> 0x8025484C
float bmVanillaVectorDistanceFromInfiniteLine(const float * start /* r31 */, const float * end /* r4 */, const float * point /* r29 */, float * closestPoint /* r30 */) {
    // Local variables
    float startPointVector[4]; // r1+0x8
    float unitLineVector[4]; // r1+0x18
    float pointDistance; // f11
    float startPointDistanceSqrd; // f1
    float startEndDistance;
    float pointDistanceSqrd; // f11

    // Range: 0x802546D8 -> 0x802546D8
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
    ...
    // Range: 0x8025473C -> 0x8025475C
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025476C -> 0x8025476C
    inline void bmVanillaVectorCDiv(float * dest, const float * src, const float val) {}
    ...
    // Range: 0x802547F4 -> 0x80254814
    inline void bmVanillaVectorCMulAdd(float * dest, const float * src, const float fmul, const float * vadd) {}

    // Range: 0x80254830 -> 0x80254830
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0x8025484C -> 0x802548D8
void bmVanillaVectorRotateX(float * dest /* r30 */, const float * src /* r4 */, const float xrads /* f29 */) {
    // Local variables
    float x; // f28
    float y; // f30
    float z; // f27
...
// Range: 0x802548D8 -> 0x80254964
void bmVanillaVectorRotateX(float * src /* r30 */, const float xrads /* f29 */) {
    // Local variables
    float x; // f28
    float y; // f30
    float z; // f27
...
// Range: 0x80254964 -> 0x802549F0
void bmVanillaVectorRotateY(float * dest /* r30 */, const float * src /* r4 */, const float yrads /* f29 */) {
    // Local variables
    float x; // f27
    float y; // f28
    float z; // f30
...
// Range: 0x802549F0 -> 0x80254A7C
void bmVanillaVectorRotateY(float * src /* r30 */, const float yrads /* f29 */) {
    // Local variables
    float x; // f27
    float y; // f28
    float z; // f30
...
// Range: 0x80254A7C -> 0x80254B08
void bmVanillaVectorRotateZ(float * dest /* r30 */, const float * src /* r4 */, const float zrads /* f29 */) {
    // Local variables
    float x; // f28
    float y; // f30
    float z; // f27
...
// Range: 0xFFFFFFFF -> 0x80254B08
void bmVanillaVectorRotateZ(float * src /* r30 */, const float zrads /* f29 */) {
    // Local variables
    float x; // f27
    float y; // f30
    float z; // f28
...
// Range: 0x80254B08 -> 0x80254BC8
void bmVanillaVectorRotateXY(float * dest /* r30 */, const float * src /* r4 */, const float xrads /* f29 */, const float yrads /* f28 */) {
    // Local variables
    float x; // f27
    float y; // f31
    float z; // f26
...
// Range: 0x80254BC8 -> 0x80254C88
void bmVanillaVectorRotateXY(float * src /* r30 */, const float xrads /* f29 */, const float yrads /* f28 */) {
    // Local variables
    float x; // f27
    float y; // f31
    float z; // f26
...
// Range: 0xFFFFFFFF -> 0x80254C88
void bmVanillaVectorRotateXZ(float * dest /* r30 */, const float * src /* r4 */, const float xrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f31
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254C88
void bmVanillaVectorRotateXZ(float * src /* r30 */, const float xrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f31
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254C88
void bmVanillaVectorRotateYX(float * dest /* r30 */, const float * src /* r4 */, const float yrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f30
    float y; // f28
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254C88
void bmVanillaVectorRotateYX(float * src /* r30 */, const float yrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f29
    float y; // f28
    float z; // f30
...
// Range: 0x80254C88 -> 0x80254D48
void bmVanillaVectorRotateYZ(float * dest /* r30 */, const float * src /* r4 */, const float yrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f29
    float z; // f30
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateYZ(float * src /* r30 */, const float yrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f29
    float z; // f30
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZX(float * dest /* r30 */, const float * src /* r4 */, const float zrads /* f28 */, const float xrads /* f27 */) {
    // Local variables
    float x; // f29
    float y; // f31
    float z; // f26
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZX(float * src /* r30 */, const float zrads /* f28 */, const float xrads /* f27 */) {
    // Local variables
    float x; // f29
    float y; // f31
    float z; // f26
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZY(float * dest /* r30 */, const float * src /* r4 */, const float zrads /* f28 */, const float yrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f30
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZY(float * src /* r30 */, const float zrads /* f28 */, const float yrads /* f27 */) {
    // Local variables
    float x; // f26
    float y; // f30
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateXYZ(float * dest /* r30 */, const float * src /* r4 */, const float xrads /* f29 */, const float yrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f26
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateXYZ(float * src /* r30 */, const float xrads /* f29 */, const float yrads /* f28 */, const float zrads /* f27 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f26
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateXZY(float * dest /* r30 */, const float * src /* r4 */, const float xrads /* f28 */, const float zrads /* f27 */, const float yrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f30
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateXZY(float * src /* r30 */, const float xrads /* f28 */, const float zrads /* f27 */, const float yrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f30
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateYXZ(float * dest /* r30 */, const float * src /* r4 */, const float yrads /* f27 */, const float xrads /* f26 */, const float zrads /* f25 */) {
    // Local variables
    float x; // f28
    float y; // f29
    float z; // f30
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateYXZ(float * src /* r30 */, const float yrads /* f27 */, const float xrads /* f26 */, const float zrads /* f25 */) {
    // Local variables
    float x; // f28
    float y; // f29
    float z; // f30
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateYZX(float * dest /* r30 */, const float * src /* r4 */, const float yrads /* f28 */, const float zrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateYZX(float * src /* r30 */, const float yrads /* f28 */, const float zrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZXY(float * dest /* r30 */, const float * src /* r4 */, const float zrads /* f28 */, const float xrads /* f27 */, const float yrads /* f26 */) {
    // Local variables
    float x; // f29
    float y; // f31
    float z; // f25
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZXY(float * src /* r30 */, const float zrads /* f28 */, const float xrads /* f27 */, const float yrads /* f26 */) {
    // Local variables
    float x; // f29
    float y; // f31
    float z; // f25
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZYX(float * dest /* r30 */, const float * src /* r4 */, const float zrads /* f28 */, const float yrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f29
...
// Range: 0xFFFFFFFF -> 0x80254D48
void bmVanillaVectorRotateZYX(float * src /* r30 */, const float zrads /* f28 */, const float yrads /* f27 */, const float xrads /* f26 */) {
    // Local variables
    float x; // f25
    float y; // f31
    float z; // f29
...
// Range: 0x80254D48 -> 0x80254DEC
float bmVanillaAngleBetweenVectors(const float * vector1 /* r3 */, const float * vector2 /* r30 */) {
    // Local variables
    float norm1[4]; // r1+0x8
    float norm2[4]; // r1+0x18
    float dot; // f1

    // Range: 0x80254D58 -> 0x80254D58
    inline float bmVanillaVectorDot(const float * src1, const float * src2) {}

    // Range: 0x80254DD4 -> 0x80254DD8
    inline float bmVanillaACos(float val) {}
}

// Range: 0x80254DEC -> 0x80254EE4
void bmVanillaVectorGetPlaneContainingPoints(float * dest /* r30 */, const float * point1 /* r29 */, const float * point2 /* r5 */, const float * point3 /* r6 */) {
    // Local variables
    float line1[4]; // r1+0x8
    float line2[4]; // r1+0x18

    // Range: 0x80254DFC -> 0x80254DFC
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80254EE4 -> 0x80254FA8
void bmVanillaQuatToMatrix(float (* dest)[4] /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f3
    float ys; // f5
    float zs; // f12
...
// Range: 0x80254FA8 -> 0x80255048
void bmVanillaQuatToMatrix33(float (* dest)[4] /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f3
    float ys; // f9
    float zs; // f0
...
// Range: 0x80255048 -> 0x8025509C
void bmVanillaQuatToForwardVector(float * dest /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f7
    float ys; // f12
    float zs; // f0
...
// Range: 0x8025509C -> 0x802550EC
void bmVanillaQuatToRightVector(float * dest /* r3 */, const float * src /* r4 */) {
    // Local variables
    float ys; // f12
    float zs; // f13
    float wy;
...
// Range: 0x802550EC -> 0x80255140
void bmVanillaQuatToUpVector(float * dest /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f0
    float ys; // f7
    float zs; // f8
...
// Range: 0x80255140 -> 0x80255340
void bmVanillaMatrixToQuat(float * dest /* r30 */, float (* src)[4] /* r31 */) {
    // Local variables
    float tr; // f10
    float s; // f1

    // Range: 0x8025517C -> 0x8025517C
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x802551E8 -> 0x802551E8
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x802551E8 -> 0x802551E8
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x8025525C -> 0x8025525C
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x802552C4 -> 0x802552C4
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0x80255340 -> 0x80255560
void bmVanillaQuatSlerp(float * dest /* r30 */, const float * src1 /* r31 */, const float * src2 /* r5 */, const float t /* f24 */) {
    // Local variables
    float omega;
    float cosom; // f1
    float sinom;
...
// Range: 0xFFFFFFFF -> 0x80255560
void bmVanillaQuatSlerpLonger(float * dest /* r30 */, const float * src1 /* r31 */, const float * src2 /* r5 */, const float t /* f24 */) {
    // Local variables
    float omega;
    float cosom; // f1
    float sinom;
...
// Range: 0x80255560 -> 0x80255640
void bmVanillaQuatMultiply(float * dest /* r3 */, const float * src1 /* r4 */, const float * src2 /* r5 */) {
    // Local variables
    float dp; // f13
}

// Range: 0x80255640 -> 0x802556A4
void bmVanillaQuatXRotation(float * quat /* r30 */, const float radians /* f1 */) {
    // Local variables
    float ang; // f31

    // Range: 0x80255654 -> 0x80255654
    inline float bmVanillaCos(float val) {}
...
// Range: 0x802556A4 -> 0x80255708
void bmVanillaQuatYRotation(float * quat /* r30 */, const float radians /* f1 */) {
    // Local variables
    float ang; // f31

    // Range: 0x802556B8 -> 0x802556B8
    inline float bmVanillaCos(float val) {}
...
// Range: 0x80255708 -> 0x8025576C
void bmVanillaQuatZRotation(float * quat /* r30 */, const float radians /* f1 */) {
    // Local variables
    float ang; // f31

    // Range: 0x8025571C -> 0x8025571C
    inline float bmVanillaCos(float val) {}
...
// Range: 0x8025576C -> 0x8025580C
void bmVanillaQuatInvToMatrix33(float (* dest)[4] /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f3
    float ys; // f9
    float zs; // f0
...
// Range: 0x8025580C -> 0x802558D0
void bmVanillaQuatInvToMatrix(float (* dest)[4] /* r3 */, const float * src /* r4 */) {
    // Local variables
    float xs; // f3
    float ys; // f5
    float zs; // f12
...
// Range: 0x802558D0 -> 0x802559D4
void bmVanillaQuatToMatrix(float (* dest)[4] /* r3 */, float (* inverse)[4] /* r4 */, const float * src /* r5 */) {
    // Local variables
    float xs; // f9
    float ys; // f5
    float zs; // f4
...
// Range: 0x802559D4 -> 0x80255A98
void bmVanillaQuatToMatrix33(float (* dest)[4] /* r3 */, float (* inverse)[4] /* r4 */, const float * src /* r5 */) {
    // Local variables
    float xs; // f10
    float ys; // f6
    float zs; // f5
...
// Range: 0x80255A98 -> 0x80255BB0
void bmVanillaQuatToRotation(float * dest /* r31 */, const float * src /* r30 */) {
    // Local variables
    float thetaOver2;
    float sinThetaOver2;
    float m; // f11
    ...
    // Range: 0x80255AE4 -> 0x80255AE4
    inline float bmVanillaACos(float val) {}

    // Range: 0x80255AE4 -> 0x80255AE4
    inline float bmVanillaSin(float val) {}

    // Range: 0x80255B0C -> 0x80255B0C
    inline float bmVanillaSqrt(float x) {}
}

// Range: 0x80255BB0 -> 0x80255C38
void bmVanillaRotationToQuat(float * dest /* r30 */, const float * src /* r29 */) {
    // Local variables
    float thetaOver2; // f31
    float sinThetaOver2;

    // Range: 0x80255BC8 -> 0x80255BC8
    inline float bmVanillaSin(float val) {}
...
// Range: 0x80255C38 -> 0x80255CD4
void bmVanillaQuatToDirection(float * dest /* r3 */, const float * src /* r4 */, float * tilt /* r31 */) {
    // Local variables
    float xs; // f13
    float ys; // f7
    float zs; // f8
    ...
    // Range: 0x80255CA8 -> 0x80255CA8
    inline float bmVanillaASin(float val) {}
}

// Range: 0x80255CD4 -> 0x80255DB8
void bmVanillaDirectionToQuat(float * quat /* r30 */, const float * direction /* r31 */, const float tilt /* f30 */) {
    // Local variables
    float f;
    float yAng; // f31
    float xAng; // f1
    float len;
    float xForm[4][4]; // r1+0x8

    // Range: 0x80255CEC -> 0x80255CEC
    inline float bmVanillaATan2(float y, float x) {}

    // Range: 0x80255D34 -> 0x80255D34
    inline float bmVanillaSqrt(float x) {}
...
// Range: 0x80255DB8 -> 0x8025600C
void bmVanillaDirectionToQuat(float * quat /* r29 */, const float * direction /* r28 */, const float * upApprox /* r30 */) {
    // Local variables
    float mat[4][4]; // r1+0x8

    // Range: 0x80255DEC -> 0x80255DEC
    inline void bmVanillaQuatCopy(float * dest, const float * src) {}

    // Range: 0x80255E0C -> 0x80255E0C
    inline void bmVanillaQuatCopy(float * dest, const float * src) {}

    // Range: 0x80255E38 -> 0x80255F00
    inline void bmVanillaDirectionToMatrix(float (* mat)[4], const float * dirIn, const float * upApprox) {
        // Local variables
        float right[4]; // r1+0x68
        float dir[4]; // r1+0x58
        float up[4]; // r1+0x48

        // Range: 0x80255E38 -> 0x80255E38
        inline void bmVanillaVectorNegate(float * src) {}

        // Range: 0x80255E74 -> 0x80255E74
        inline void bmVanillaVectorCross(float * dest, const float * src1, const float * src2) {
            // Local variables
            float tmpDest[4]; // r1+0x78
        }

        // Range: 0x80255EB4 -> 0x80255EB4
        inline float bmVanillaFAbs(float x) {}

        // Range: 0x80255ED4 -> 0x80255ED4
        inline void bmVanillaVectorCrossX(float * dest, const float * src) {}

        // Range: 0x80255EEC -> 0x80255F00
        inline void bmVanillaVectorCrossYNeg(float * dest, const float * src) {}

        // Range: 0x80255F00 -> 0x80255F00
        inline void bmVanillaVectorCross(float * dest, const float * src1, const float * src2) {
            // Local variables
            float tmpDest[4]; // r1+0x78
        }
...
// Range: 0x8025600C -> 0x80256050
void bmVanillaYDirectionToQuat(float * quat /* r30 */, const float * direction /* r4 */, const float tilt /* f1 */) {
    // Local variables
    float qZtoY[4]; // @ 0x80385060
    float qZ[4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80256050
void bmVanillaYDirectionToQuat(float * quat /* r29 */, const float * up /* r31 */, const float * dirApprox /* r5 */) {
    // Local variables
    float mat[4][4]; // r1+0x8

    // Range: 0x80256050 -> 0x80256050
    inline void bmVanillaQuatCopy(float * dest, const float * src) {}
    ...
    inline void bmVanillaYDirectionToMatrix(float (* mat)[4], const float * up, const float * dirApprox) {
        // Local variables
        float right[4]; // r1+0x58
        float dir[4]; // r1+0x48

        // Range: 0x80256050 -> 0x80256050
        inline void bmVanillaVectorCross(float * dest, const float * src1, const float * src2) {
            // Local variables
            float tmpDest[4]; // r1+0x68
        }
...
// Range: 0x80256050 -> 0x80256114
void bmVanillaQuatNorm(float * dest /* r30 */, const float * source /* r31 */) {
    // Local variables
    float dist; // f12
    float square; // f1

    // Range: 0x80256098 -> 0x80256098
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x802560D8 -> 0x80256100
    inline void bmVanillaQuatCopy(float * dest, const float * src) {}
}

// Range: 0x80256114 -> 0x802561DC
void bmVanillaQuatNormApprox(float * dest /* r30 */, const float * source /* r31 */) {
    // Local variables
    float dist; // f12
    float square; // f1

    // Range: 0x80256160 -> 0x80256160
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x802561A0 -> 0x802561C8
    inline void bmVanillaQuatCopy(float * dest, const float * src) {}
}

// Range: 0x802561DC -> 0x80256560
void bmVanillaQuatSlerpApprox(float * dest /* r29 */, const float * src1 /* r4 */, const float * src2 /* r5 */, float t /* f30 */) {
    // Local variables
    float pdist; // f13
    float ndist; // f0
    float xd; // f8
    ...
    float lower; // f28
    float mid; // f31
    float qUpper[4]; // r1+0x8
    float qLower[4]; // r1+0x18

    // Range: 0x80256270 -> 0x80256270
    inline void bmVanillaVectorNegate4(float * dest, const float * src) {}

    // Range: 0x8025629C -> 0x802562B4
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x802562B4 -> 0x802562B4
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x802562E4 -> 0x802562E4
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}

    // Range: 0x80256340 -> 0x80256340
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}

    // Range: 0x802563A4 -> 0x802563A4
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}

    // Range: 0x802563F8 -> 0x802563F8
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}

    // Range: 0x80256460 -> 0x80256460
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}

    // Range: 0x802564B4 -> 0x802564B4
    inline void bmVanillaVectorAdd4(float * src1, const float * src2) {}
}

// Range: 0x80256560 -> 0x8025663C
void bmVanillaQuatLerp(float * dest /* r31 */, const float * src1 /* r4 */, const float * src2 /* r5 */, float t /* f31 */) {
    // Local variables
    float pdist; // f13
    float ndist; // f0
    float xd; // f3
...
// Range: 0x8025663C -> 0x80256718
void bmVanillaQuatLerpApprox(float * dest /* r31 */, const float * src1 /* r4 */, const float * src2 /* r5 */, float t /* f31 */) {
    // Local variables
    float pdist; // f13
    float ndist; // f0
    float xd; // f3
...
// Range: 0x80256718 -> 0x80256808
void bmVanillaEulerToQuatYXZ(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f31
    float y2; // f29
    float z2; // f27
...
// Range: 0x80256808 -> 0x802568F8
void bmVanillaEulerToQuatYZX(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f31
    float y2; // f29
    float z2; // f26
...
// Range: 0x802568F8 -> 0x802569E8
void bmVanillaEulerToQuatXYZ(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f30
    float y2; // f29
    float z2; // f27
...
// Range: 0xFFFFFFFF -> 0x802569E8
void bmVanillaEulerToQuatXZY(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f31
    float y2; // f27
    float z2; // f25
...
// Range: 0x802569E8 -> 0x80256AE0
void bmVanillaEulerToQuatZXY(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f31
    float y2; // f27
    float z2; // f26
...
// Range: 0x80256AE0 -> 0x80256BD8
void bmVanillaEulerToQuatZYX(float * quat /* r30 */, float x /* f1 */, float y /* f2 */, float z /* f3 */) {
    // Local variables
    float x2; // f31
    float y2; // f27
    float z2; // f25
...
// Range: 0x80256BD8 -> 0x80256C64
void bmVanillaQuatToEulerYXZ(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0x80256C64 -> 0x80256CF0
void bmVanillaQuatToEulerYZX(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0x80256CF0 -> 0x80256D7C
void bmVanillaQuatToEulerXYZ(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80256D7C
void bmVanillaQuatToEulerXZY(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0x80256D7C -> 0x80256E08
void bmVanillaQuatToEulerZXY(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0xFFFFFFFF -> 0x80256E08
void bmVanillaQuatToEulerZYX(float * quat /* r4 */, float * x /* r31 */, float * y /* r30 */, float * z /* r29 */) {
    // Local variables
    float m[4][4]; // r1+0x8
}

// Range: 0x80256E08 -> 0x80256E98
float bmVanillaQuatDot(const float * src1 /* r3 */, const float * src2 /* r4 */) {
    // Local variables
    float tempFloat; // f9
    float tempFloat2; // f10
}

// Range: 0xFFFFFFFF -> 0x80256E98
int bmVanillaQuatDelta(float * delta /* r11 */, const float * end /* r4 */, const float * start /* r5 */) {
    // Local variables
    float diffLenSquared;
    float sumLenSquared;
    float diff[4]; // r1+0x8
    float sum[4]; // r1+0x18

    // Range: 0x80256E98 -> 0x80256E98
    inline void bmVanillaVectorSub4(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x80256E98 -> 0x80256F7C
float bmVanillaQuatSlerpAtFixedSpeed(float * dest /* r30 */, const float * src1 /* r29 */, const float * src2 /* r31 */, const float speed /* f30 */) {
    // Local variables
    float dot; // f1
    float ang;

    // Range: 0x80256EFC -> 0x80256EFC
    inline float bmVanillaACos(float val) {}
...
// Range: 0xFFFFFFFF -> 0x80256F7C
void bmVanillaQuatCMul(float * dest /* r30 */, float factor /* f31 */) {
    // Local variables
    float rotation[4]; // r1+0x8
}

// Range: 0x80256F7C -> 0x80257004
void bmVanillaQuatCMulAdd(float * dest /* r30 */, float * src /* r4 */, float factor /* f31 */) {
    // Local variables
    float temp[4]; // r1+0x8
    float rotation[4]; // r1+0x18

    // Range: 0x80256F90 -> 0x80256FEC
    inline void bmVanillaQuatMultiply(float * src1, const float * src2) {
        // Local variables
        float temp[4]; // r1+0x28

        // Range: 0x80256F90 -> 0x80256F90
        inline void bmVanillaQuatCopy(float * dest, const float * src) {}
    }
}

// Range: 0x80257004 -> 0x802570F0
void bmVanillaQuatRotateVector2(float * dest /* r3 */, const float * q /* r4 */, const float * v /* r5 */) {
    // Local variables
    float xs; // f0
    float ys; // f4
    float zs; // f3
...
// Range: 0x802570F0 -> 0x802571C8
void bmVanillaQuatRotateVector(const float * q /* r3 */, float * v /* r4 */) {
    // Local variables
    float xs; // f31
    float ys; // f9
    float zs; // f12
...
// Range: 0x802571C8 -> 0x802571CC
void bInitMaths() {}

// Range: 0x802571CC -> 0x802571D0
void bShutdownMaths() {}

// Range: 0xFFFFFFFF -> 0x802571D0
int bmGeomIntersectLines(const struct _TBCollisionLine * const line1 /* r3 */, const struct _TBCollisionLine * const line2 /* r4 */, float * const t /* r5 */, float * const s /* r6 */) {
    // Local variables
    float line1Vector[4]; // r1+0x8
    float line2Vector[4]; // r1+0x18
    float crossV1V2[4]; // r1+0x28
    float squareMagnitude;

    // Range: 0x802571D0 -> 0x802571D0
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0xFFFFFFFF -> 0x802571D0
float bmGeomLengthAlongVector(const float * basis /* r3 */, const float * test /* r4 */) {
    // Local variables
    float basisLen;
...
// Range: 0x802571D0 -> 0x802572D4
void bmGCMatMultiply44(float (* dest)[4] /* r3 */, float (* src1)[4] /* r4 */, float (* src2)[4] /* r5 */) {}

// Range: 0x802572D4 -> 0x802573D8
void bmGCMatMultiply44(float (* src1)[4] /* r3 */, float (* src2)[4] /* r4 */) {}

// Range: 0x802573D8 -> 0x8025742C
void bmGCMatMultiplyVector(float (* mat)[4] /* r3 */, float * vec /* r4 */) {}

// Range: 0x8025742C -> 0x80257480
void bmGCMatMultiplyVector2(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {}

// Range: 0x80257480 -> 0x802574C4
void bmGCMatMultiply33Vector(float (* mat)[4] /* r3 */, float * vec /* r4 */) {}

// Range: 0x802574C4 -> 0x80257508
void bmGCMatMultiply33Vector2(float * dest /* r3 */, float (* mat)[4] /* r4 */, const float * vec /* r5 */) {}

// Range: 0x80257508 -> 0x8025756C
void bmGCMatTranspose(float (* dest)[4] /* r3 */, float (* src)[4] /* r4 */) {}
...
unsigned int bHoldRand; // size: 0x4, address: 0x8047CF68
typedef float TBVector[4];
typedef TBVector TBQuaternion;
const TBVector bIdentityVector; // size: 0x10, address: 0x80377850
const TBVector bXAxisVector; // size: 0x10, address: 0x80377860
const TBVector bNegXAxisVector; // size: 0x10, address: 0x80377870
const TBVector bYAxisVector; // size: 0x10, address: 0x80377880
const TBVector bNegYAxisVector; // size: 0x10, address: 0x80377890
const TBVector bZAxisVector; // size: 0x10, address: 0x803778A0
const TBVector bNegZAxisVector; // size: 0x10, address: 0x803778B0
typedef float TBVector2D[2];
typedef Mtx44 TBMatrix;
Mtx44 bIdentityMatrix; // size: 0x40, address: 0x80384FB0
Mtx44 bZeroMatrix; // size: 0x40, address: 0x80384FF0
TBVector bIdentityQuaternion; // size: 0x10, address: 0x80385030
TBVector bUpQuaternion; // size: 0x10, address: 0x80385040
TBVector bDownQuaternion; // size: 0x10, address: 0x80385050
// total size: 0x44
