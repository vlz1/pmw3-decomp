/*
    Compile unit: b:/BlitzSDK/Babel/GameCube/Src/bSound/bSoundSourceFiles.cpp
    Producer: GNU C++ 2.95.3 SN BUILD v1.62 for Nintendo Gamecube
    Compile directory: b:\BlitzSDK\Babel\GameCube\Src\bSound
    Language: C++
    Code range: 0x8025756C -> 0x80260C6C
    GCC Source File Name Offset: 0x0001BD96
    GCC Source Info Offset: 0x00050964
*/
...
// Range: 0x8025756C -> 0x802575C0
int bInitSound() {}

// Range: 0x802575C0 -> 0x802575E4
void bShutdownSound() {}

// Range: 0x802575E4 -> 0x802575F4
static void bARAMSetupCompleteCallback(unsigned int task /* r3 */) {}

// Range: 0x802575F4 -> 0x80257604
static void bARAMUploadCompleteCallback(unsigned int task /* r3 */) {
    // Local variables
    struct _TBARAMRequestInfo * request;
}

// total size: 0x34
struct _TBARAMRequestInfo {
    // Members
    struct ARQRequest task; // offset 0x0, size 0x20
    unsigned int flags; // offset 0x20, size 0x4
    char * event; // offset 0x24, size 0x4
    void * context; // offset 0x28, size 0x4
    struct _TBPackageIndex * package; // offset 0x2C, size 0x4
    struct _TBARAMRequestInfo * next; // offset 0x30, size 0x4
};
// total size: 0x20
struct ARQRequest {
    // Members
...
// Range: 0x80257604 -> 0x802576BC
void bARAMHeapReset(struct _TBARAMHeap * heap /* r3 */) {}

// Range: 0x802576BC -> 0x80257760
static void bARAMConnectFreeBlock(struct _TARAMBlock * block /* r3 */) {
    // Local variables
    struct _TARAMBlock * freeBlock; // r10
}

// Range: 0x80257760 -> 0x80257764
static void bARAMVMLogCallback(unsigned int realVirtualAddress /* r3 */, unsigned int physicalAddress /* r4 */, unsigned int pageNo /* r5 */, unsigned int pageMissLatency /* r6 */, int pageSwappedOut /* r7 */) {}

// Range: 0x80257764 -> 0x80257868
int bInitVirtualMemory(unsigned int flags /* r30 */) {}

// Range: 0x80257868 -> 0x802579EC
int bARAMInitHeap() {
    // Local variables
    unsigned char * zero; // r30
}

// Range: 0x802579EC -> 0x80257B3C
int bARAMShutdownHeap(struct _TBARAMHeap * heap /* r31 */) {
    // Local variables
    struct _TARAMBlock * block; // r30
}

// Range: 0x80257B3C -> 0x80257D68
struct _TARAMBlock * bARAMHeapAlloc(unsigned int size /* r29 */, unsigned short flags /* r27 */) {
    // Local variables
    struct _TARAMBlock * block; // r31
    struct _TARAMBlock * newBlock; // r30
    struct _TARAMBlock * delBlock;
    int old; // r28
}

// Range: 0x80257D68 -> 0x80257EAC
void bARAMHeapFree(struct _TARAMBlock * block /* r30 */) {
    // Local variables
    struct _TARAMBlock * prevBlock;
    struct _TARAMBlock * nextBlock;
    struct _TARAMBlock * delBlock; // r10
    struct _TBARAMHeap * curHeap; // r28
    int old; // r29
}

// Range: 0x80257EAC -> 0x80257F08
int bkARAMHeapFreeSpace(int * largestFreeBlock /* r3 */) {
    // Local variables
    struct _TARAMBlock * block; // r11
    int size; // r9
    int max; // r10
}

// Range: 0xFFFFFFFF -> 0x80257F08
void bkARAMHeapShow(int pause /* r3 */) {}

// Range: 0x80257F08 -> 0x80257FDC
static int bARAMFreeCompletedRequests() {
    // Local variables
    struct _TBARAMRequestInfo * remove; // r3
    struct _TBARAMRequestInfo * request; // r31
    struct _TBARAMRequestInfo * last; // r29
    int active; // r28
}

// Range: 0x80257FDC -> 0x80258128
struct _TBARAMHeap * bkARAMCreateHeap(unsigned int size /* r30 */) {
    // Local variables
    struct _TBARAMHeap * curHeap; // r28
    struct _TBARAMHeap * reserved; // r31
    struct _TARAMBlock * rootBlock; // r8
    struct _TARAMBlock * block; // r30
}

// Range: 0x80258128 -> 0x80258174
void bkARAMDeleteHeap(struct _TBARAMHeap * heap /* r30 */) {}

// Range: 0x80258174 -> 0x80258190
void bkARAMSetActiveHeap(struct _TBARAMHeap * heap /* r3 */) {}

// Range: 0x80258190 -> 0x80258294
int bkARAMUpload(void * buffer /* r25 */, struct _TARAMBlock * block /* r30 */, char * event /* r24 */, int offset /* r27 */, int size /* r7 */, struct _TBPackageIndex * index /* r31 */, int priority /* r28 */, void * context /* r26 */) {
    // Local variables
    struct _TBARAMRequestInfo * request; // r5
    int owner; // r0
    int totalSize; // r29
    unsigned int mram;
    unsigned int aram; // r30
    int arqPriority; // r23
}

// Range: 0xFFFFFFFF -> 0x80258294
static void bARAMDownloadCompleteCallback(unsigned int task /* r3 */) {
    // Local variables
    struct _TBARAMRequestInfo * request;
}

// Range: 0xFFFFFFFF -> 0x80258294
int bkARAMDownload(void * buffer /* r27 */, struct _TARAMBlock * block /* r31 */, char * event /* r24 */, int offset /* r28 */, int size /* r7 */, int priority /* r29 */, void * context /* r25 */) {
    // Local variables
    struct _TBARAMRequestInfo * request; // r11
    int owner; // r0
    int totalSize; // r30
    unsigned int mram;
    unsigned int aram; // r31
    int arqPriority; // r26
}

// Range: 0xFFFFFFFF -> 0x80258294
static void bARAMSwapCompleteCallback(unsigned int task /* r3 */) {
    // Local variables
    struct _TBARAMRequestInfo * request;
}

// Range: 0xFFFFFFFF -> 0x80258294
static void bARAMSwapContinueCallback(unsigned int task /* r3 */) {
    // Local variables
    struct _TBARAMRequestInfo * request; // r11
    unsigned int owner;
    unsigned int requestAt;
    unsigned int offset; // r8
    unsigned int copySize; // r31
    unsigned int remainingSize; // r27
    unsigned int mram; // r25
    unsigned int aram; // r30
    unsigned int buff; // r29
    unsigned int abuf; // r26
}

// Range: 0xFFFFFFFF -> 0x80258294
int bkARAMSwap(void * buffer /* r28 */, struct _TARAMBlock * block /* r31 */, char * event /* r22 */, int priority /* r24 */, void * context /* r21 */) {
    // Local variables
    struct _TBARAMRequestInfo * request; // r31
    int owner; // r0
    int requestAt;
    unsigned int copySize; // r30
    unsigned int size; // r29
    unsigned int offset;
    unsigned int mram;
    unsigned int aram; // r26
    unsigned int buff; // r23
    unsigned int abuf; // r27
}

// Range: 0x80258294 -> 0x802582B4
void bUpdateARAM() {
    // Local variables
    int active;
}

// Range: 0x802582B4 -> 0x8025832C
int bInitSpecificMusic() {
    // Local variables
    int c;
}

// Range: 0xFFFFFFFF -> 0x8025832C
void bLoadAudioStreamSpecific(struct _TBAudioStream * stream /* r3 */) {}

// Range: 0x8025832C -> 0x8025842C
struct _TBAudioStream * bLoadAudioStreamByCRC(struct _TBPackageIndex * index /* r31 */, unsigned int crc /* r4 */) {
    // Local variables
    struct _TBAudioStream * stream; // r11
    int subStream; // r10
    int channel; // ctr
}

// Range: 0x8025842C -> 0x802585EC
int bLoadAudioStreamArray(struct _TBPackageIndex * index /* r29 */) {
    // Local variables
    struct _TBAudioStream * stream; // r31
    struct _TBAudioStream * firstStream; // r25
    int strCount; // r3
    int subStream; // r11
    int channel; // ctr
    unsigned int arrayNameCRC; // r26
}

// Range: 0x802585EC -> 0x80258750
void bDeleteAudioStream(struct _TBAudioStream * stream /* r31 */) {
    // Local variables
    unsigned int arrayNameCRC; // r30

    /* anonymous block */ {
        // Range: 0x8025862C -> 0x80258728
        struct _TBAudioStream * curStream; // r7
    }
}

// Range: 0x80258750 -> 0x80258A3C
static void GetNextBlock(struct _TBAudioStream * stream /* r31 */) {
    // Local variables
    int blockSize; // r28
    int dataSize; // r25
    int packetSize; // r27
    int packetsPerBlock; // r10
    int readSize; // r24
    unsigned char * loadAddr; // r23
    int subStream; // r30
}

// Range: 0x80258A3C -> 0x80258BDC
static void DMACompleteCallback(char * name /* r3 */, char * parms /* r4 */, void * data /* r5 */, void * context /* r6 */) {
    // Local variables
    struct _TBAudioStream * stream; // r31
}

// Range: 0x80258BDC -> 0x80258D08
static void UploadToARAM(struct _TBAudioStream * stream /* r31 */) {
    // Local variables
    int subStream; // r26
    int channel; // r30
    int bufferSize; // r21
...
// Range: 0x80258D08 -> 0x80258D94
static void BlockLoadedCallback(char * name /* r3 */, char * parms /* r4 */, void * data /* r5 */, void * context /* r6 */) {
    // Local variables
    struct _TBAudioStream * stream; // r31
    struct _TBBkgLoadResult * loadResult;
}
...
// Range: 0x80258D94 -> 0x80258DD8
static void bDropStreamCallback(void * p /* r3 */) {
    // Local variables
    struct _AXVPB * pvpb;
    unsigned int id; // r11
    unsigned int stream; // r0
    unsigned int subStream; // r9
    unsigned int channel;
}

// Range: 0x80258DD8 -> 0x80258EF4
static void GetChannelVolumePan(struct _TBAudioStream * stream /* r3 */, int subStream /* r4 */, int channel /* r5 */, int * volume /* r6 */, int * pan /* r7 */) {
    // Local variables
    int babelVolume; // r10
    int channelVolume; // r4
    int mixerPan; // r4
}

// Range: 0x80258EF4 -> 0x80259310
static int CreateSoundBuffer(struct _TBAudioStream * stream /* r26 */) {
    // Local variables
    ...
    int end; // r0
    int loop; // r9
    int channelPan; // r1+0x2C
    int channelVolume; // r1+0x28
    struct _AXPBADDR addr; // r1+0x8
    struct _AXPBSRC src; // r1+0x18
    int blockSize; // r0
    int channelSize; // r0
    int aramSize; // r20
    struct _AXPBADPCM * adpcm;
    struct _AXPBADPCMLOOP * adpcmLoop; // r30
    float ratio;
    unsigned int id;
    int old; // r18
}

// Range: 0x80259310 -> 0x8025941C
static void DeleteSoundBuffer(struct _TBAudioStream * stream /* r27 */) {
    // Local variables
    int subStream; // r11
    int channel; // r28
    int old; // r24
...
// Range: 0x8025941C -> 0x802594B0
static void PlaySoundBuffer(struct _TBAudioStream * stream /* r30 */) {
    // Local variables
    int subStream; // r9
    int channel; // r29
}

// Range: 0x802594B0 -> 0x80259544
static void StopSoundBuffer(struct _TBAudioStream * stream /* r30 */) {
    // Local variables
    int subStream; // r9
    int channel; // r29
}

// Range: 0x80259544 -> 0x8025993C
static int bPlayAudioStreamADPCM(struct _TBAudioStream * stream /* r31 */, unsigned int flags /* r4 */, unsigned int volume /* r20 */, int pan /* r18 */, unsigned int frequency /* r21 */, void (* callback)(struct _TBAudioStream *, enum EBSoundStreamCallbackReason, void *, void *) /* r17 */, void * context /* r16 */) {
    // Local variables
    char buf[256]; // r1+0x8
    int l; // r11
    int subStream; // r7
...
// Range: 0x8025993C -> 0x80259998
int bsPlayAudioStream(struct _TBAudioStream * stream /* r3 */, unsigned int flags /* r4 */, unsigned int volume /* r5 */, int pan /* r6 */, unsigned int frequency /* r7 */, void (* callback)(struct _TBAudioStream *, enum EBSoundStreamCallbackReason, void *, void *) /* r8 */, void * context /* r9 */, int startSamplePos /* r10 */) {}

// Range: 0x80259998 -> 0x80259BA8
void bsStopAudioStream(struct _TBAudioStream * stream /* r31 */) {
    // Local variables
    int status; // r3
    char diskError[2]; // @ 0x8047D718
}

// Range: 0x80259BA8 -> 0x80259BF0
void bStopAllAudioStreams() {
    // Local variables
    int c;
}

// Range: 0xFFFFFFFF -> 0x80259BF0
int bsAllocAudioStreamData(struct _TBAudioStream * stream /* r31 */) {
    // Local variables
    int blockSize; // r9
    int dataSize;
}

// Range: 0x80259BF0 -> 0x80259C48
void bsFreeAudioStreamData(struct _TBAudioStream * stream /* r31 */) {}

// Range: 0x80259C48 -> 0x80259DFC
int bsSetAudioStreamVolume(struct _TBAudioStream * stream /* r29 */, unsigned int volume /* r24 */, int pan /* r23 */, int subStream /* r26 */) {
    // Local variables
    int channelPan; // r1+0xC
    int channelVolume; // r1+0x8
    int channel; // r27
}

// Range: 0xFFFFFFFF -> 0x80259DFC
void bsSetGlobalAudioStreamVolume(int volume /* r3 */) {
    // Local variables
    struct _TBAudioStream * stream;
    int mixerVolume; // r27
    int c; // r9
    int subStream; // r8
    int channel; // r29
...
// Range: 0x80259DFC -> 0x80259F38
int bsSetAudioStreamFrequency(struct _TBAudioStream * stream /* r29 */, unsigned int frequency /* r4 */) {
    // Local variables
    float ratio; // f31
    int subStream; // r6
    int channel; // r30
...
// Range: 0x80259F38 -> 0x80259FA8
int bsSetAudioStreamFlags(struct _TBAudioStream * stream /* r31 */, unsigned int flags /* r30 */) {}

// Range: 0x80259FA8 -> 0x8025A038
int bsClearAudioStreamFlags(struct _TBAudioStream * stream /* r31 */, unsigned int flags /* r30 */) {}

// Range: 0xFFFFFFFF -> 0x8025A038
int bsGetAudioStreamFlags(struct _TBAudioStream * stream /* r3 */, unsigned int flag /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A038
int bsSeekAudioStream(struct _TBAudioStream * stream /* r3 */, float seconds /* f1 */) {}

// Range: 0x8025A038 -> 0x8025A21C
void bNotifyADPCM() {
    // Local variables
    struct _TBAudioStream * stream; // r31
    unsigned int start; // r9
    unsigned int at; // r0
    unsigned int soundBufferOffset; // r10
    unsigned int bufferSize; // r7
    unsigned int channelSize; // r0
    unsigned int thisOffset; // r9
    unsigned int nextOffset; // r0
    int c;
}

// Range: 0x8025A21C -> 0x8025A400
void bUpdateMusic() {
    // Local variables
    struct _TBAudioStream * stream;
    int s; // r30
...
// Range: 0xFFFFFFFF -> 0x8025A400
int bsGetAudioStreamPosition(struct _TBAudioStream * stream /* r7 */) {
    // Local variables
    unsigned int start; // r11
    unsigned int at; // r9
    unsigned int bufferSize;
    unsigned int blockSize; // r9
    unsigned int channelSize; // r3
    unsigned int soundBufferOffset; // r11
    unsigned int diskOffset; // r0
}

// Range: 0xFFFFFFFF -> 0x8025A400
void bsGetAudioStreamInfo(struct _TBAudioStream * stream /* r3 */, int * origFrequency /* r4 */, int * noofSamples /* r5 */, int * bitDepth /* r6 */, int * compressionRatio /* r7 */, int * noofMarkers /* r8 */, int * * markers /* r9 */) {}

// Range: 0xFFFFFFFF -> 0x8025A400
void bGetAudioStreamBufferInfo(struct _TBAudioStream * stream /* r3 */, int * memBufSize /* r4 */, int * memBuffers /* r5 */, int * soundBufSize /* r6 */, int * soundBuffers /* r7 */, int * diskPos /* r8 */, int * memPos /* r31 */, int * soundPos /* r29 */, int * loading /* r28 */) {
    // Local variables
    unsigned int blockSize; // r11
    unsigned int bufferSize; // r12
    unsigned int channelSize; // r30
    unsigned int start; // r9
    unsigned int at; // r0
    unsigned int diskOffset; // r9
    unsigned int soundBufferOffset; // r11
}

// Range: 0xFFFFFFFF -> 0x8025A400
enum EBSoundStreamStatus bsGetAudioStreamStatus(struct _TBAudioStream * stream /* r3 */, int * volume /* r4 */, int * pan /* r5 */, int * frequency /* r6 */, int subStream /* r7 */) {}

// Range: 0xFFFFFFFF -> 0x8025A400
int bsFindAudioStreamMarker(struct _TBAudioStream * stream /* r9 */, int sampleOffset /* r4 */) {
    // Local variables
    int first; // r8
    int last; // r11
    int current; // r3
...
// Range: 0x8025A400 -> 0x8025A4AC
int bsSetAudioStreamData(struct _TBAudioStream * stream /* r30 */, void * data /* r31 */) {
    // Local variables
    int totalBytesRequired; // r29

    /* anonymous block */ {
        // Range: 0x8025A454 -> 0x8025A46C
        int dataSize;
    }
}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListEnum(void (* callback)(struct TBPlayListCallbackInfo *, void *) /* r3 */, void * context /* r4 */) {}
...
// Range: 0xFFFFFFFF -> 0x8025A4AC
struct _TBPlayList * bsPlayListSelect(int list /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListPlay(struct _TBPlayList * playlist /* r3 */, int song /* r4 */, unsigned int flags /* r5 */, unsigned int volume /* r6 */, int pan /* r7 */, unsigned int frequency /* r8 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
void bPlayListUpdate() {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
void bsPlayListStop(struct _TBPlayList * playlist /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
void bsPlayListDelete(struct _TBPlayList * playlist /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListSetVolume(struct _TBPlayList * playlist /* r3 */, unsigned int volume /* r4 */, int pan /* r5 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListSetFrequency(struct _TBPlayList * playlist /* r3 */, unsigned int frequency /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListFlagsSet(struct _TBPlayList * playlist /* r3 */, unsigned int flags /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListFlagsClear(struct _TBPlayList * playlist /* r3 */, unsigned int flags /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListFlagsGet(struct _TBPlayList * playlist /* r3 */, unsigned int flag /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
unsigned short * bsPlayListGetSongName(struct _TBPlayList * playlist /* r3 */, int song /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
int bsPlayListGetNumberOfSongs(struct _TBPlayList * playlist /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025A4AC
unsigned short * bsPlayListGetName(struct _TBPlayList * playlist /* r3 */) {}
...
// Range: 0xFFFFFFFF -> 0x8025A4AC
enum EBPlayListStatus bsPlayListGetStatus(struct _TBPlayList * playlist /* r3 */, int * volume /* r4 */, int * pan /* r5 */, int * frequency /* r6 */) {}

// Range: 0x8025A4AC -> 0x8025A4D0
static void bAXCallback() {}

// Range: 0x8025A4D0 -> 0x8025A4DC
static void WaveDMACompleteCallback(char * name /* r3 */, char * parms /* r4 */, void * data /* r5 */, void * context /* r6 */) {
    // Local variables
    struct _TBSample * sample;
}
...
// Range: 0x8025A4DC -> 0x8025A614
int bInitSpecificWave() {}

// Range: 0x8025A614 -> 0x8025A658
void bShutdownSpecificWave() {}

// Range: 0x8025A658 -> 0x8025A6C0
static void bDropVoiceCallback(void * p /* r3 */) {
    // Local variables
    struct _AXVPB * pvpb;
    unsigned int channel; // r0
}

// Range: 0x8025A6C0 -> 0x8025A808
void bUpdateChannelVolume(int channel /* r29 */) {
    // Local variables
    int mixerVolume; // r30
    int mixerPan; // r4
    int old; // r28
...
// Range: 0x8025A808 -> 0x8025A940
void bSetChannelFrequency(int channel /* r29 */, int frequency /* r30 */) {
    // Local variables
    int old; // r28
    float newFrequency; // f13
    float ratio;
}

// Range: 0x8025A940 -> 0x8025AB54
struct _TBSample * bLoadSampleByCRC(struct _TBPackageIndex * index /* r27 */, unsigned int crc /* r28 */) {
    // Local variables
    struct _TSampleHeader * header; // r31
    struct _TBSample * sample; // r30
    void * data; // r29
    int totalSampleLength; // r31
    int owner;
}

// total size: 0x20
struct _TSampleHeader {
    // Members
    int noofSamples; // offset 0x0, size 0x4
    int frequency; // offset 0x4, size 0x4
    unsigned short bitDepth; // offset 0x8, size 0x2
    unsigned short noofChannels; // offset 0xA, size 0x2
    unsigned int flags; // offset 0xC, size 0x4
    int groupId; // offset 0x10, size 0x4
    int encoded; // offset 0x14, size 0x4
    int pad2; // offset 0x18, size 0x4
    int pad3; // offset 0x1C, size 0x4
};
// Range: 0x8025AB54 -> 0x8025B1DC
int bPlaySample(struct _TBSample * sample /* r27 */, int volume /* r29 */, int pan /* r28 */, int frequency /* r24 */, unsigned int flags /* r26 */, int channel /* r30 */) {
    // Local variables
    struct _AXPBADDR addr; // r1+0x8
    struct _AXPBSRC src; // r1+0x18
    unsigned int priority;
    ...
    int mixerPan; // r8
    int old; // r23
    float newFrequency; // f9
    float ratio; // f12
    unsigned int sampleAddr; // r8
    unsigned int silenceAddr; // r4
    int auxAAttenuation; // r6
    int auxBAttenuation; // r7

    /* anonymous block */ {
        // Range: 0x8025B14C -> 0x8025B174
        struct _AXPBADPCM * adpcm;
        struct _AXPBADPCMLOOP * loop; // r29
    }
}

// Range: 0x8025B1DC -> 0x8025B2EC
void bStopChannel(int channel /* r29 */) {
    // Local variables
    int mixerVolume;
    int old; // r28
}

// Range: 0x8025B2EC -> 0x8025B368
void bDeleteSample(struct _TBSample * sample /* r31 */) {}

// Range: 0x8025B368 -> 0x8025B42C
void bUpdateSoundChannelStatus(int channel /* r30 */) {
    // Local variables
    int old; // r29
}

// Range: 0x8025B42C -> 0x8025B574
void bPauseChannel(int channel /* r30 */, int pauseStatus /* r29 */) {
    // Local variables
    unsigned int aramSilenceEndInNibbles; // r9
    int old; // r28
...
// Range: 0x8025B574 -> 0x8025B6AC
int bPlaySample3D(struct _TBSample * sample /* r29 */, float * pos /* r27 */, int volume /* r28 */, int frequency /* r30 */, unsigned int flags /* r24 */, int channel /* r25 */, float minDistance /* f30 */, float maxDistance /* f31 */) {
    // Range: 0x8025B620 -> 0x8025B620
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
...
// Range: 0xFFFFFFFF -> 0x8025B6AC
void bsSet3DDistanceFactor(float scaleToMeters /* f1 */) {}

// Range: 0x8025B6AC -> 0x8025BA48
void bUpdate3DSoundPosition(int channel /* r31 */, float * oldPos /* r29 */, float * newPos /* r30 */) {
    // Local variables
    float posInView[4]; // r1+0x8
    ...
    float range; // f13
    int old; // r27
    float closingSpeed; // f0
    int mixerVolume; // r0
    float diff[4]; // r1+0x28

    // Range: 0x8025B738 -> 0x8025B738
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B754 -> 0x8025B754
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B780 -> 0x8025B780
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B7A0 -> 0x8025B7A0
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B7C0 -> 0x8025B7C0
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B7C0 -> 0x8025B7C0
    inline float bmVanillaATan(float val) {}

    // Range: 0x8025B7E4 -> 0x8025B7E4
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x8025B85C -> 0x8025B85C
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}
...
// Range: 0x8025BA48 -> 0x8025BA4C
void bFinaliseUpdated3DSounds() {}

// Range: 0x8025BA4C -> 0x8025BB40
int bsSetSoundMode(enum EBSoundMode soundMode /* r31 */) {
    // Local variables
    struct TBGlobalReverbSetting reverbInfo; // r1+0x8
    int restartReverb; // r30
}
...
// Range: 0xFFFFFFFF -> 0x8025BB40
enum EBSoundMode bsGetSoundMode() {}

// Range: 0x8025BB40 -> 0x8025BB70
void * bAXFXAllocHook(unsigned int size /* r3 */) {}

// Range: 0x8025BB70 -> 0x8025BB90
void bAXFXFreeHook(void * ptr /* r3 */) {}

// Range: 0x8025BB90 -> 0x8025BD58
void bsEnableGlobalReverb(struct TBGlobalReverbSetting * reverbInfo /* r30 */) {
    // Local variables
    int ret;
    int type; // r29
    int level; // r28
...
// Range: 0x8025BD58 -> 0x8025BDC8
void bsDisableGlobalReverb() {}

// Range: 0x8025BDC8 -> 0x8025BEB4
void bsSetGlobalReverbLevel(int level /* r3 */) {
    // Local variables
    int noofSampleChannels;

    /* anonymous block */ {
        // Range: 0x8025BDE4 -> 0x8025BEA0
        int i;
    }
}

// Range: 0x8025BEB4 -> 0x8025BEE8
void bsGetGlobalReverb(struct TBGlobalReverbSetting * reverbInfo /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025BEE8
int bsIsGlobalReverbEnabled() {}

// Range: 0xFFFFFFFF -> 0x8025BEE8
void bsEnableGlobalDelay(struct TBGlobalDelaySetting * delayInfo /* r30 */) {
    // Local variables
    int ret;
    int level; // r27
    float delay; // f31
...
// Range: 0xFFFFFFFF -> 0x8025BEE8
void bsDisableGlobalDelay() {}

// Range: 0xFFFFFFFF -> 0x8025BEE8
void bsSetGlobalDelayLevel(int level /* r3 */) {
    // Local variables
    int noofSampleChannels;

    /* anonymous block */ {
        // Range: 0x8025BEE8 -> 0x8025BEE8
        int i;
    }
}

// Range: 0xFFFFFFFF -> 0x8025BEE8
void bsGetGlobalDelay(struct TBGlobalDelaySetting * delayInfo /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025BEE8
int bsIsGlobalDelayEnabled() {}

// Range: 0x8025BEE8 -> 0x8025BF88
struct _TBLipSync * bLoadLipSyncByCRC(struct _TBPackageIndex * pakIndex /* r3 */, unsigned int crc /* r4 */) {
    // Local variables
    struct _TBLipSync * lipSync; // r7

    // Range: 0x8025BF74 -> 0x8025BF74
    inline void bsSetLipSyncPseudoFrequencies(struct _TBLipSync * lipSync, float f1, float f2, float f3) {}
}

// Range: 0x8025BF88 -> 0x8025C0B0
int bLoadLipSyncArray(struct _TBPackageIndex * index /* r29 */) {
    // Local variables
    struct _TBLipSync * lipsync; // r31
    struct _TBLipSync * firstLipsync; // r25
    int lipCount; // r27
    unsigned int arrayNameCRC; // r30

    // Range: 0x8025C000 -> 0x8025C000
    inline void bsSetLipSyncPseudoFrequencies(struct _TBLipSync * lipSync, float f1, float f2, float f3) {}
}

// Range: 0x8025C0B0 -> 0x8025C16C
void bDeleteLipSync(struct _TBLipSync * lipSync /* r31 */) {
    // Local variables
    unsigned int arrayNameCRC; // r3

    /* anonymous block */ {
        // Range: 0x8025C0DC -> 0x8025C144
        struct _TBLipSync * curLipsync; // r11
    }
}

// Range: 0xFFFFFFFF -> 0x8025C16C
void bsSetupLipSyncController(struct _TBLipSyncController * controller /* r30 */, int flags /* r29 */, enum EBLipSyncRotationOrder order /* r28 */, float xaMult /* f25 */, float yaMult /* f24 */, float zaMult /* f26 */, float xMult /* f27 */, float yMult /* f28 */, float zMult /* f31 */, float xaThresh /* f30 */, float yaThresh /* f29 */, float zaThresh /* f23 */) {}
...
struct _TBLipSyncController {
    // Inner declarations
    union {
        float angularMultiplier[4]; // offset 0x0, size 0x10
        unsigned int target[4]; // offset 0x0, size 0x10
    };
    union {
        float positionMultiplier[4]; // offset 0x0, size 0x10
        float targetMultiplier[4]; // offset 0x0, size 0x10
    };

    // Members
    struct _TBLipSync * lipSync; // offset 0x0, size 0x4
    ...
    float threshold[4]; // offset 0x30, size 0x10
    int (* userController)(struct _TBActorInstance *, struct _TBActorNodeInstance *, void *); // offset 0x40, size 0x4
    union UBContext32 userContext; // offset 0x44, size 0x4
...
// Range: 0xFFFFFFFF -> 0x8025C16C
void bsSetupLipSyncTargetController(struct _TBLipSyncController * controller /* r30 */, int flags /* r29 */, int target /* r28 */, float multiplier /* f30 */, float threshold /* f29 */, int target2 /* r27 */, float multiplier2 /* f28 */, float threshold2 /* f31 */) {}

// Range: 0xFFFFFFFF -> 0x8025C16C
void bsAttachLipSyncToNode(struct _TBLipSync * lipSync /* r3 */, struct _TBActorNodeInstance * node /* r29 */, struct _TBLipSyncController * controller /* r30 */) {}

// Range: 0xFFFFFFFF -> 0x8025C16C
int bsRemoveLipSyncFromNode(struct _TBActorNodeInstance * node /* r30 */, struct _TBLipSyncController * controller /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025C16C
int bsPauseLipSync(struct _TBLipSyncController * controller /* r31 */, int pauseStatus /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8025C16C
static int bGetLipSyncPosition(struct _TBLipSyncController * controller /* r31 */) {
    // Local variables
    int frames; // r9
    unsigned long long pauseTime; // r27
    unsigned long long adjustedStartTime; // r29
    struct _TBLipSync * lipSync; // r26
}

// Range: 0xFFFFFFFF -> 0x8025C16C
int bsGetLipSyncPosition(struct _TBLipSyncController * controller /* r3 */) {
    // Local variables
    int frames; // r0
}

// Range: 0x8025C16C -> 0x8025C4C0
int bsGetLipSyncData(struct _TBLipSync * lipSync /* r29 */, float time1024 /* f30 */, int * vol /* r27 */, int * freq /* r26 */, int * levels /* r25 */) {
    // Local variables
    unsigned int dftDataSize; // r9
    unsigned int c; // r10
    unsigned char * data; // r11
    unsigned int time60; // r28
    int value; // r0

    /* anonymous block */ {
        // Range: 0x8025C214 -> 0x8025C35C
        float fTmp; // f31
        float fAccum; // f29
        int myVol; // r0
        /* anonymous block */ {
            // Range: 0x8025C240 -> 0x8025C2B4
            int f; // r30

            // Range: 0x8025C26C -> 0x8025C26C
            inline float bmVanillaSin(float val) {}
        }

        // Range: 0x8025C2B4 -> 0x8025C2B4
        inline float bmVanillaFAbs(float x) {}
    }
}

// Range: 0xFFFFFFFF -> 0x8025C4C0
int bLipSyncNodeController(struct _TBActorInstance * actorInstance /* r27 */, struct _TBActorNodeInstance * node /* r29 */, void * context /* r5 */) {
    // Local variables
    struct _TBLipSyncController * lipController; // r31
    struct _TBLipSync * lipSync; // r30
    unsigned char * data; // r5
    int frames; // r7
    float lipSyncValue; // f11
    float lipSyncFreqValue; // f0
    float angle[4]; // r1+0x8
    float orientation[4]; // r1+0x18
    float position[4]; // r1+0x28
    int c;
    int n; // r5
    unsigned char targets[2]; // r1+0x40
    float weights[2]; // r1+0x48
    int morphFlags; // r8
    int dftDataSize; // r3

    /* anonymous block */ {
        // Range: 0x8025C4C0 -> 0x8025C4C0
        int vol; // r1+0x38
        int freq; // r1+0x3C
    }
}
...
// Range: 0x8025C4C0 -> 0x8025C80C
struct _TBSoundBank * bLoadSoundBankByCRC(struct _TBPackageIndex * pakIndex /* r17 */, unsigned int crc /* r4 */) {
    // Local variables
    struct _TBSoundBank * loadedBank; // r31
    struct _TBSoundBank * bank; // r27
    struct _TBSoundPatch * curPatch; // r24
    struct _TBSoundPart * curPart; // r31
    struct _TBSoundTrack * curTrack; // r26
    struct _TBSoundSegment * curSeg; // r29
    struct _TBSoundPatchRef * curPatchRef; // r30
    int i; // r9
    int j; // r30
    int k; // r9
    int l; // r11
    ...
    // Range: 0x8025C658 -> 0x8025C658
    inline struct _TBSoundPatch * bkFindSoundPatchByCRC(unsigned int crc, struct _TBPackageID pak, unsigned int group, unsigned int flags) {}

    // Range: 0x8025C710 -> 0x8025C710
    inline struct _TBSample * bkFindSampleByCRC(unsigned int crc, struct _TBPackageID pak, unsigned int group, unsigned int flags) {}
}
...
// Range: 0x8025C80C -> 0x8025C838
void bDeleteSoundBank(struct _TBSoundBank * bank /* r3 */) {}

// Range: 0x8025C838 -> 0x8025C840
struct _TBSoundPatch * bLoadSoundPatchByCRC(struct _TBPackageIndex * pakIndex /* r3 */, unsigned int crc /* r4 */) {}

// Range: 0x8025C840 -> 0x8025C844
void bDeleteSoundPatch(struct _TBSoundPatch * bank /* r3 */) {}

// Range: 0x8025C844 -> 0x8025C86C
int bInitPatchPart(struct _TBSoundPartInstance * part /* r11 */) {}

// total size: 0xCC
struct _TBSoundPartInstance {
    // Members
    struct _TBSoundPatch3dInfo info3d; // offset 0x0, size 0x3C
    ...
    struct _TBSoundPatch * patch; // offset 0x90, size 0x4
    int * refCount; // offset 0x94, size 0x4
    int flags; // offset 0x98, size 0x4
    struct _TBSample * sample; // offset 0x9C, size 0x4
    int channelID; // offset 0xA0, size 0x4
    struct _TBSoundPatchUserParams userParams; // offset 0xA4, size 0x28
};
// total size: 0x3C
...
// Range: 0x8025C86C -> 0x8025CAB8
int bStartPatchPart(struct _TBSoundPartInstance * part /* r31 */) {}

// Range: 0x8025CAB8 -> 0x8025CB20
int bStopPatchPart(struct _TBSoundPartInstance * part /* r31 */) {}

// Range: 0x8025CB20 -> 0x8025CB5C
int bPausePatchPart(struct _TBSoundPartInstance * part /* r3 */) {}

// Range: 0x8025CB5C -> 0x8025CB98
int bUnpausePatchPart(struct _TBSoundPartInstance * part /* r3 */) {}

// Range: 0x8025CB98 -> 0x8025CDD4
int bUpdatePatchPart(struct _TBSoundPartInstance * part /* r30 */) {
    // Local variables
    int chVol; // r8
    int chPan; // r31
}

// Range: 0x8025CDD4 -> 0x8025CEA8
unsigned int bsPlayPositionalSoundPatch(struct _TBSoundPatch * patch /* r3 */, float * pos /* r4 */, int volume /* r12 */, int frequency /* r8 */, unsigned int flags /* r7 */, float minDistance /* f1 */, float maxDistance /* f2 */, int * refCount /* r8 */) {
    // Local variables
    struct _TBSoundPatch3dInfo info3d; // r1+0x8
    float xDist; // f12
    float yDist; // f13
    float zDist; // f0

    // Range: 0x8025CE94 -> 0x8025CE94
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0x8025CEA8 -> 0x8025CF4C
unsigned int bsPlayTrackingSoundPatch(struct _TBSoundPatch * patch /* r3 */, float * pos /* r10 */, int volume /* r29 */, int frequency /* r30 */, unsigned int flags /* r0 */, void (* callback)(float *, int, void *) /* r8 */, void * context /* r9 */, float minDistance /* f1 */, float maxDistance /* f2 */, int * refCount /* r10 */) {
    // Local variables
    struct _TBSoundPatch3dInfo info3d; // r1+0x8

    // Range: 0x8025CEB8 -> 0x8025CEB8
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0x8025CF4C -> 0x8025CFE8
void bsSetPatchRange(unsigned int patchID /* r29 */, float minDistance /* f30 */, float maxDistance /* f31 */) {
    // Local variables
    int i; // r30
    int status;
}

// Range: 0x8025CFE8 -> 0x8025D084
void bsSetPatchPitchShift(unsigned int patchID /* r29 */, float minPitchShift /* f30 */, float maxPitchShift /* f31 */) {
    // Local variables
    int i; // r30
    int status;
}

// Range: 0x8025D084 -> 0x8025D138
void bsSetPatchVolume(unsigned int patchID /* r30 */, int volume /* r31 */) {
    // Local variables
    int i;
    int status;
}

// Range: 0x8025D138 -> 0x8025D1FC
void bsSetPatchPitchShift(unsigned int patchID /* r31 */, float pitchShift /* f31 */) {
    // Local variables
    int i;
    int status;
}

// Range: 0x8025D1FC -> 0x8025D3C4
int bAllowPatchPlay(struct _TBSoundPartInstance * newInstance /* r30 */) {
    // Local variables
    struct _TBSoundPartInstance * curInstance; // r31
    int i; // r29
}

// Range: 0x8025D3C4 -> 0x8025D5AC
int bFindChannelToCull(struct _TBSoundPartInstance * newInstance /* r29 */) {
    // Local variables
    struct _TBSoundPartInstance * curInstance; // r8
    struct _TBSoundPartInstance * cullList[48]; // r1+0x8
    int i; // r8
    int numCulls; // r6
    int swapped; // r30
}

// Range: 0x8025D5AC -> 0x8025D6AC
float bSoundPatchShiftPitch(float frequency /* f1 */, float shiftBy /* f2 */) {
    // Local variables
    float scalar; // f12
    int semiTone; // r8
    float semiInterp; // f2
...
// Range: 0x8025D6AC -> 0x8025D740
void bsResetSoundPatches() {
    // Local variables
    int i;
}

// Range: 0x8025D740 -> 0x8025DA4C
unsigned int bsPlaySoundPatch(struct _TBSoundPatch * patch /* r30 */, int volume /* r20 */, int pan /* r21 */, int frequency /* r22 */, unsigned int flags /* r26 */, struct _TBSoundPatch3dInfo * info3d /* r27 */, int * refCount /* r24 */) {
    // Local variables
    unsigned int ret; // r25
    struct _TBSoundPart * curPart; // r28
    struct _TBSoundPartInstance * curInstance; // r31
    struct _TBSoundSequenceInstance * curSeqInstance; // r31
    int i; // r23
    int j; // r29

    /* anonymous block */ {
        // Range: 0x8025D894 -> 0x8025DA34
        int prevTotal; // r19
    }
}
...
struct _TBSoundSequenceInstance {
    ...
    struct _TBSoundPatch * sequence; // offset 0x1C, size 0x4
    struct _TBSoundTrackInstance tracks[64]; // offset 0x20, size 0xB00
    struct _TBSoundPatch3dInfo info3d; // offset 0xB20, size 0x3C
...
// Range: 0x8025DA4C -> 0x8025DD3C
void bInitPartInstance(struct _TBSoundPartInstance * curInstance /* r29 */, struct _TBSoundPart * curPart /* r31 */) {
    // Local variables
    int flags; // r30
    int rnd; // r3
    float fPitch; // f30
    ...
    /* anonymous block */ {
        // Range: 0x8025DAAC -> 0x8025DB18
        int randVolume; // r30
    }

    /* anonymous block */ {
        // Range: 0x8025DB84 -> 0x8025DC88
        float randPitch; // f31
    }

    /* anonymous block */ {
        // Range: 0x8025DCB0 -> 0x8025DD1C
        int randPan; // r30
    }
}

// Range: 0x8025DD3C -> 0x8025E558
int bUpdateSoundPatches(unsigned int milliseconds /* r19 */) {
    // Local variables
    struct _TBSoundSequenceInstance * curSeqInstance; // r29
    struct _TBSoundPartInstance * curInstance; // r30
    struct _TBSoundPart * curPart; // r4
    int i; // r21
    int j; // r6
    int k; // r28
    int l; // r7
    ...
    int isLooping;
    int segDelay; // r9
    int segDelayRange; // r8
    float fTime; // f31
    float fDelta; // f12

    /* anonymous block */ {
        // Range: 0x8025DEA4 -> 0x8025E19C
        struct _TBSoundTrackInstance * trk; // r31

        // Range: 0x8025DF38 -> 0x8025DF38
        inline int bmVanillaRand() {}

        // Range: 0x8025DFC0 -> 0x8025DFC0
        inline int bmVanillaRand() {}
        /* anonymous block */ {
            // Range: 0x8025E034 -> 0x8025E19C
            struct _TBSoundPatchRef * ref; // r30
            unsigned int saveHandle; // r25

            // Range: 0x8025E0CC -> 0x8025E0CC
            inline int bmVanillaRand() {}
            /* anonymous block */ {
                // Range: 0x8025E10C -> 0x8025E168
                unsigned int ph;
            }
        }
    }
    ...
    /* anonymous block */ {
        // Range: 0x8025E440 -> 0x8025E4C0
        int canStart; // r31
    }
}

// Range: 0x8025E558 -> 0x8025E66C
int bsStopSoundPatch(unsigned int handle /* r28 */) {
    // Local variables
    struct _TBSoundSequenceInstance * curSeqInstance; // r8
    struct _TBSoundPartInstance * curInstance; // r31
    int i; // r30
    int numStopped; // r29
}

// Range: 0xFFFFFFFF -> 0x8025E66C
int bsStopSoundPatch(struct _TBSoundPatch * patch /* r30 */) {
    // Local variables
    struct _TBSoundSequenceInstance * curSeqInstance; // r31
    struct _TBSoundPartInstance * curInstance; // r31
    int i; // r29
...
// Range: 0x8025E66C -> 0x8025E690
int bsStopAllSoundPatches() {}

// Range: 0x8025E690 -> 0x8025E7C4
int bsPauseSoundPatch(unsigned int handle /* r28 */, int pause /* r4 */) {
    // Local variables
    struct _TBSoundSequenceInstance * curSeqInstance; // r9
    struct _TBSoundPartInstance * curInstance; // r31
    int i; // r29
    int numAffected; // r30
}

// Range: 0x8025E7C4 -> 0x8025E7EC
int bsPauseAllSoundPatches(int pause /* r4 */) {}

// Range: 0x8025E7EC -> 0x8025EEB0
int bEvaluateSoundEnvelope(struct _TBSoundPartInstance * instance /* r31 */, float milliseconds /* f1 */, int paramType /* r30 */) {
    // Local variables
    struct _TBSoundPart * part; // r27
    struct _TBSoundEnvelope * envelope; // r29
    float fNode; // f30
    float fLFO; // f7
    float fVolume; // f11
    char validNode; // r26
    ...
    float fPitch; // f10
    int semiTone; // r11
    float semiInterp; // f12
    bool userLfoLevel; // r11
    bool userLfoFrequency; // r6
    bool userEnvLevel; // r10

    // Range: 0x8025EA68 -> 0x8025EA68
    inline float bmVanillaSin(float val) {}

    // Range: 0x8025EA80 -> 0x8025EA80
    inline float bmVanillaFAbs(float x) {}

    // Range: 0x8025EA84 -> 0x8025EA84
    inline float bmVanillaFAbs(float x) {}

    // Range: 0x8025EAC0 -> 0x8025EAC0
    inline float bmVanillaFAbs(float x) {}

    // Range: 0x8025EAF4 -> 0x8025EAF4
    inline float bmVanillaFAbs(float x) {}

    // Range: 0x8025EB7C -> 0x8025EB98
    inline float bmVanillaFAbs(float x) {}

    /* anonymous block */ {
        // Range: 0x8025EE2C -> 0x8025EE78
        float fPan; // f13
    }
}

// Range: 0xFFFFFFFF -> 0x8025EEB0
int bsGetActiveSoundPatchChannels() {
    // Local variables
    int i;
    int numPlaying; // r3
}
...
// Range: 0xFFFFFFFF -> 0x8025EEB0
int bsGetActiveSequences() {
    // Local variables
    int i;
    int numPlaying; // r3
}

// Range: 0x8025EEB0 -> 0x8025EFC4
int bsGetSoundPatchIDStatus(unsigned int patchID /* r10 */) {
    // Local variables
    int i;
    int noofPresent; // r11
    int noofPlaying;
...
// Range: 0xFFFFFFFF -> 0x8025EFC4
int bsIsSoundPatchActive(struct _TBSoundPatch * patch /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8025EFC4
int bsSetSoundPatchUserParams(unsigned int handle /* r6 */, struct _TBSoundPatchUserParams * params /* r4 */) {
    // Local variables
    struct _TBSoundSequenceInstance * curSeqInstance;
    struct _TBSoundPartInstance * curInstance; // r11
    int i; // r10
    int noofModified; // r3
}

// Range: 0x8025EFC4 -> 0x8025F050
int bMillibelsToLinearLevel(int mb /* r10 */) {
    // Local variables
    int current; // r3
    int half; // r8
}

// Range: 0xFFFFFFFF -> 0x8025F050
int bLinearLevelToMillibels(int level /* r3 */) {}

// Range: 0x8025F050 -> 0x8025F088
int bInitGenericWave() {}

// Range: 0x8025F088 -> 0x8025F0A8
void bShutdownGenericWave() {}

// Range: 0x8025F0A8 -> 0x8025F244
float CalculatePositionalVolumes(float * pos /* r31 */, int * volume /* r29 */, int * pan /* r30 */, float minDistance /* f29 */, float maxDistance /* f31 */) {
    // Local variables
    float xDist; // f0
    float yDist; // f13
    float zDist; // f1
    ...
    float vLen; // f1
    float distFactor; // f1
    float viewPos[4]; // r1+0x8

    // Range: 0x8025F0C4 -> 0x8025F0C4
    inline float bmVanillaSqrt(float x) {}

    // Range: 0x8025F14C -> 0x8025F14C
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x8025F14C -> 0x8025F14C
        inline float bmVanillaSqrt(float x) {}
    }
...
// Range: 0x8025F244 -> 0x8025F3E8
static int ProcessPlaybackFlags(struct _TBSample * sample /* r29 */, unsigned int flags /* r4 */) {
    // Local variables
    int l; // r31
}

// Range: 0x8025F3E8 -> 0x8025F474
static int FindIdleChannel(struct _TBSample * sample /* r3 */, unsigned int flags /* r4 */) {
    // Local variables
    int l; // r30
}

// Range: 0x8025F474 -> 0x8025F49C
unsigned int CombineFlags(struct _TBSample * sample /* r3 */, unsigned int flags /* r4 */) {
    // Local variables
    unsigned int newFlags; // r3
}

// Range: 0x8025F49C -> 0x8025F564
int bsPlaySample(struct _TBSample * sample /* r29 */, int volume /* r28 */, int pan /* r26 */, int frequency /* r27 */, unsigned int flags /* r7 */) {
    // Local variables
    int channel; // r31
}

// Range: 0x8025F564 -> 0x8025F78C
int bsPlayPositionalSample(struct _TBSample * sample /* r28 */, float * pos /* r31 */, int volume /* r27 */, int frequency /* r23 */, unsigned int flags /* r29 */, float minDistance /* f31 */, float maxDistance /* f30 */) {
    // Local variables
    int channel; // r30
    int distFactor; // r1+0x8
    int dVolume; // r25
    int pan; // r1+0xC

    // Range: 0x8025F600 -> 0x8025F600
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025F600 -> 0x8025F600
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025F6F8 -> 0x8025F6F8
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0x8025F78C -> 0x8025F9B8
int bsPlayTrackingSample(struct _TBSample * sample /* r30 */, float * pos /* r31 */, int volume /* r27 */, int frequency /* r25 */, unsigned int flags /* r26 */, void (* callback)(float *, int, void *) /* r23 */, void * context /* r22 */, float minDistance /* f31 */, float maxDistance /* f30 */) {
    // Local variables
    int channel; // r28
    int distFactor; // r1+0x8
    int dVolume; // r24
    int pan; // r1+0xC
    struct _TBSoundChannelInfo * chInfo; // r11

    // Range: 0x8025F94C -> 0x8025F94C
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025F974 -> 0x8025F974
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}
...
struct _TBSoundChannelInfo {
    // Members
    float position[4]; // offset 0x0, size 0x10
    float oldPosition[4]; // offset 0x10, size 0x10
    enum EBSoundChannelStatus status; // offset 0x20, size 0x4
    struct _TBSample * sample; // offset 0x24, size 0x4
    ...
    struct _TBSoundChannelInfoHAL hal; // offset 0x68, size 0x74
};
...
struct _TBSoundChannelInfoHAL {
    // Members
    struct _AXVPB * soundBuffer; // offset 0x0, size 0x4
    struct AXART_SOUND axartSound; // offset 0x4, size 0x14
    struct AXART_3D axart3d; // offset 0x18, size 0x2C
    struct AXART_PITCH axartPitch; // offset 0x44, size 0xC
    struct AXART_VOLUME axartVolume; // offset 0x50, size 0xC
    struct AXART_AUXA_VOLUME axartAuxAVolume; // offset 0x5C, size 0xC
    struct AXART_AUXB_VOLUME axartAuxBVolume; // offset 0x68, size 0xC
};
// total size: 0x14
struct AXART_SOUND {
    // Typedefs
    ...
    void * next; // offset 0x0, size 0x4
    void * prev; // offset 0x4, size 0x4
    struct _AXVPB * axvpb; // offset 0x8, size 0x4
    float sampleRate; // offset 0xC, size 0x4
    struct AXART_ART * articulators; // offset 0x10, size 0x4
};
// total size: 0x2C
struct AXART_3D {
    // Typedefs
    ...
    // Members
    struct AXART_ART art; // offset 0x0, size 0x8
...
// Range: 0x8025F9B8 -> 0x8025FA9C
int bsPlaySampleOnChannel(int channel /* r30 */, struct _TBSample * sample /* r29 */, int volume /* r28 */, int pan /* r26 */, int frequency /* r27 */, unsigned int flags /* r4 */) {}

// Range: 0x8025FA9C -> 0x8025FCCC
int bsPlayPositionalSampleOnChannel(int channel /* r29 */, struct _TBSample * sample /* r28 */, float * pos /* r31 */, int volume /* r27 */, int frequency /* r23 */, unsigned int flags /* r30 */, float minDistance /* f31 */, float maxDistance /* f30 */) {
    // Local variables
    int distFactor; // r1+0x8
    int dVolume; // r26
    int pan; // r1+0xC

    // Range: 0x8025FB24 -> 0x8025FB24
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025FB24 -> 0x8025FB24
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x8025FC34 -> 0x8025FC34
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0x8025FCCC -> 0x8025FEFC
int bsPlayTrackingSampleOnChannel(int channel /* r25 */, struct _TBSample * sample /* r31 */, float * pos /* r30 */, int volume /* r27 */, int frequency /* r29 */, unsigned int flags /* r28 */, void (* callback)(float *, int, void *) /* r23 */, void * context /* r22 */, float minDistance /* f31 */, float maxDistance /* f30 */) {
    // Local variables
    int distFactor; // r1+0x8
    int dVolume; // r29
    int pan; // r1+0xC
    ...
    // Range: 0x8025FEAC -> 0x8025FEAC
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
}

// Range: 0xFFFFFFFF -> 0x8025FEFC
...
// Range: 0xFFFFFFFF -> 0x8025FEFC
void bsReserveChannel(int channel /* r3 */, int newState /* r4 */) {}

// Range: 0x8025FEFC -> 0x8025FF70
void bsSetGlobalSampleVolume(int volume /* r3 */) {
    // Local variables
    int l; // r31
}

// Range: 0x8025FF70 -> 0x8025FFE0
void bsStopChannel(int channel /* r30 */) {}

// Range: 0x8025FFE0 -> 0x80260050
void bsStopAllChannels() {
    // Local variables
    int l; // r31
}

// Range: 0x80260050 -> 0x802600D8
int bsStopSample(const struct _TBSample * const sample /* r28 */) {
    // Local variables
    int i; // r31
    int noofChannels; // r29
}

// Range: 0xFFFFFFFF -> 0x802600D8
void bsStopSampleGroup(const unsigned short groupId /* r29 */) {
    // Local variables
    int i; // r31
}

// Range: 0x802600D8 -> 0x80260158
void bsPauseChannel(int channel /* r31 */, int pauseState /* r29 */) {}

// Range: 0x80260158 -> 0x80260230
void bsPauseAllChannels(int pauseState /* r3 */) {
    // Local variables
    int l; // r31
}

// Range: 0xFFFFFFFF -> 0x80260230
void bsPauseSample(struct _TBSample * sample /* r28 */, int pauseState /* r29 */) {
    // Local variables
    int i; // r31
}

// Range: 0xFFFFFFFF -> 0x80260230
void bsPauseSampleGroup(unsigned short groupId /* r28 */, int pauseState /* r30 */) {
    // Local variables
    int i; // r31
}

// Range: 0x80260230 -> 0x8026027C
void bsSetChannelVolume(int channel /* r3 */, int volume /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x8026027C
int bsGetChannelVolume(int channel /* r3 */) {}

// Range: 0x8026027C -> 0x802602C4
void bsSetChannelPan(int channel /* r3 */, int pan /* r4 */) {}

// Range: 0xFFFFFFFF -> 0x802602C4
int bsGetChannelPan(int channel /* r3 */) {}

// Range: 0x802602C4 -> 0x80260310
void bsSetChannelFrequency(int channel /* r3 */, int frequency /* r10 */) {}

// Range: 0x80260310 -> 0x80260344
int bsGetChannelFrequency(int channel /* r3 */) {}

// Range: 0x80260344 -> 0x80260860
void bsUpdate() {
    // Local variables
    int l; // r30
    int distFactor; // r1+0x18
    int pan; // r1+0x1C
    float pitchFactor; // f31
    struct _TBSoundChannelInfo * chInfo; // r28
    float vDiff[4]; // r1+0x8
    float fDist;
    float fRange; // f13

    // Range: 0x8026044C -> 0x8026049C
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x80260540 -> 0x80260564
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x80260564 -> 0x80260564
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}

    // Range: 0x80260618 -> 0x80260618
    inline int bsIsLinearFalloff() {}

    // Range: 0x80260630 -> 0x80260630
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}

    // Range: 0x80260668 -> 0x802606A0
    inline void bmVanillaVectorSub(float * dest, const float * src1, const float * src2) {}

    // Range: 0x802606A0 -> 0x802606A0
    inline float bmVanillaVectorLen(const float * src) {
        // Range: 0x802606A0 -> 0x802606A0
        inline float bmVanillaSqrt(float x) {}
    }

    // Range: 0x802606E4 -> 0x802606E4
    inline float bmVanillaLog10(float value) {}

    // Range: 0x80260700 -> 0x80260700
    inline float bmVanillaLog10(float value) {}
}

// Range: 0x80260860 -> 0x802609A0
void bsSetChannelRange(int channel /* r30 */, float minDistance /* f1 */, float maxDistance /* f2 */) {
    // Local variables
    int distFactor; // r1+0x8
    int dVolume; // r0
    int pan; // r1+0xC
    float pitchFactor; // f2
}

// Range: 0x802609A0 -> 0x80260A9C
void bsSetChannelPitchShift(int channel /* r30 */, float minPitchShift /* f13 */, float maxPitchShift /* f0 */) {
    // Local variables
    int distFactor; // r1+0x8
    int pan; // r1+0xC
    float pitchFactor; // f2
...
// Range: 0x80260A9C -> 0x80260AF8
enum EBSoundChannelStatus bsGetChannelStatus(int channel /* r31 */) {}

// Range: 0xFFFFFFFF -> 0x80260AF8
void bsSetSampleFlags(struct _TBSample * sample /* r3 */, unsigned int flagMask /* r4 */, unsigned int newStates /* r5 */) {}

// Range: 0x80260AF8 -> 0x80260C6C
void bsSetListener(float (* listenerTransform)[4] /* r3 */) {
    // Local variables
    float mTmp[4][4]; // r1+0x8

    // Range: 0x80260BD4 -> 0x80260BD4
    inline void bmVanillaVectorCopy(float * dest, const float * src) {}
...
char bHomeDirectory[256]; // size: 0x100
unsigned int bBkInitFlags; // size: 0x4
enum EBModuleId {
...
const char bPackage_ext[]; // size: 0x0
enum EBResourceType {
    EBRESTYPE_NONE = 255,
    EBRESTYPE_TEXTURE = 0,
    EBRESTYPE_ACTOR = 1,
...
const TBVector bIdentityVector; // size: 0x10
...
unsigned int aramAddrSilence; // size: 0x4, address: 0x8047D71C
int aramVMSwapSize; // size: 0x4, address: 0x8047CF74
int aramVMSize; // size: 0x4, address: 0x8047CF78
void * aramVM; // size: 0x4, address: 0x8047D720
...
int bByteToDecibelLUT[256]; // size: 0x400, address: 0x80385070
TBSoundInfo bSoundInfo; // size: 0x10, address: 0x8042E510
...
int bGlobalStreamVolume; // size: 0x4, address: 0x8047CF7C
int bNoofActiveAudioStreams; // size: 0x4, address: 0x8047D724
...
TBSoundChannelInfo bSoundChannel[64]; // size: 0x3800, address: 0x8042E520
int bGlobalSampleVolume; // size: 0x4, address: 0x8047CFC8
TBVector bSoundListenerPosition; // size: 0x10, address: 0x80431D20
Mtx44 bSoundListenerOrientation; // size: 0x40, address: 0x80431D30
Mtx44 bSoundListenerMatrix; // size: 0x40, address: 0x80431D70
int bSoundLinearFalloff; // size: 0x4, address: 0x8047CFD0
float bSoundRolloffFactor; // size: 0x4, address: 0x8047CFD4
...
enum VM_PageReplacementPolicy {
    VM_PRP_LRU = 0,
    VM_PRP_RANDOM = 1,
    VM_PRP_FIFO = 2,
};
typedef enum VM_PageReplacementPolicy VM_PageReplacementPolicy;
typedef void (* VM_LogStats)(unsigned long, unsigned long, unsigned long, unsigned long, int);
typedef struct _TBARAMRequestInfo TBARAMRequestInfo;
// total size: 0x118
struct _TBARAMDMAInfo {
    // Members
    ARQRequest request[8]; // offset 0x0, size 0x100
    unsigned int flags; // offset 0x100, size 0x4
    char * event; // offset 0x104, size 0x4
    unsigned char * buffer; // offset 0x108, size 0x4
    TARAMBlock * block; // offset 0x10C, size 0x4
    unsigned int totalSize; // offset 0x110, size 0x4
    unsigned int requestAt; // offset 0x114, size 0x4
};
typedef struct _TBARAMDMAInfo TBARAMDMAInfo;
static unsigned long aramMemArray[2]; // size: 0x8, address: 0x8047D728
static unsigned int aramAddr; // size: 0x4, address: 0x8047D730
static unsigned int aramSize; // size: 0x4, address: 0x8047D734
static TBARAMHeap aramHeap; // size: 0x60, address: 0x80431DB0
static TBARAMHeap * activeHeap; // size: 0x4, address: 0x8047CF70
static TBARAMDMAInfo * aramDMAInfo; // size: 0x4, address: 0x8047D738
static unsigned char * memSwapBuffer; // size: 0x4, address: 0x8047D73C
static TARAMBlock * aramSwapBuffer; // size: 0x4, address: 0x8047D740
static TBARAMRequestInfo bARAMRequestList; // size: 0x34, address: 0x80431E10
static TBARAMRequestInfo * swapLastRequest; // size: 0x4, address: 0x8047D744
static int swapPriority; // size: 0x4, address: 0x8047D748
TBAudioStream * bActiveAudioStream[2]; // size: 0x8, address: 0x8047D74C
char * bAudioBkgLoadEventName[2]; // size: 0x8, address: 0x8047CF80
char * bAudioBkgDMAEventName[2]; // size: 0x8, address: 0x8047CF88
unsigned long long bStreamLastUpdateTime[2]; // size: 0x10, address: 0x80431E48
int bStreamNextMarkerEx[2]; // size: 0x8, address: 0x8047D754
char * bStreamArrayName; // size: 0x4, address: 0x8047CF90
typedef struct _TSampleHeader TSampleHeader;
static float soundRange; // size: 0x4, address: 0x8047D75C
int bInterruptCode; // size: 0x4, address: 0x8047CF94
static TBEventClient * dmaCompleteEvent; // size: 0x4, address: 0x8047D760
static int bGlobalReverbInitialised; // size: 0x4, address: 0x8047CF98
static int bGlobalReverbEnabled; // size: 0x4, address: 0x8047CF9C
static int bGlobalReverbType; // size: 0x4, address: 0x8047CFA0
static int bGlobalReverbLevel; // size: 0x4, address: 0x8047CFA4
TBGlobalReverbSetting bGlobalReverbSetting; // size: 0x10, address: 0x80431E58
static int bGlobalDelayInitialised; // size: 0x4, address: 0xFFFFFFFF
static int bGlobalDelayEnabled; // size: 0x4, address: 0x8047CFAC
static int bGlobalDelayLevel; // size: 0x4, address: 0x8047CFB0
TBGlobalDelaySetting bGlobalDelaySetting; // size: 0x10, address: 0x80431E68
static AXFX_REVERBHI_DPL2 globalReverbPL2; // size: 0x26C, address: 0x80431E78
static AXFX_REVERBSTD globalReverb; // size: 0x154, address: 0x804320E4
static AXFX_DELAY globalDelay; // size: 0x60, address: 0x80432238
// total size: 0x14
struct TBGCReverbParams {
    // Members
    float time; // offset 0x0, size 0x4
    float preDelay; // offset 0x4, size 0x4
    float damping; // offset 0x8, size 0x4
    float coloration; // offset 0xC, size 0x4
    float mix; // offset 0x10, size 0x4
};
typedef struct TBGCReverbParams TBGCReverbParams;
TBGCReverbParams reverbParams[39]; // size: 0x30C, address: 0x80385470
char * bLipSyncArrayName; // size: 0x4, address: 0x8047CFB4
static float bLipSyncDefaultFrequencies[3]; // size: 0xC, address: 0x8038577C
TBSoundPartInstance bPlayingParts[48]; // size: 0x2640, address: 0x80432298
TBSoundSequenceInstance bPlayingSequences[32]; // size: 0x16E00, address: 0x804348D8
static float pitchShiftLookups[49]; // size: 0xC4, address: 0x80385788
int bSoundPatchesInited; // size: 0x4, address: 0x8047CFB8
int bNumPlayingParts; // size: 0x4, address: 0x8047CFBC
int bNumPlayingSequences; // size: 0x4, address: 0x8047CFC0
static unsigned int bSoundPartHandle; // size: 0x4, address: 0x8047CFC4
int bAutoListener; // size: 0x4, address: 0x8047CFCC
static int noofSampleChannels; // size: 0x4, address: 0x8047CFD8
int bLinearToMillibelsLUT[256]; // size: 0x400, address: 0x8038584C
