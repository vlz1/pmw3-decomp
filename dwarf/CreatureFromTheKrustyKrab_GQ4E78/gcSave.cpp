/*
    Compile unit: b:/BlitzSDK/Babel/GameCube/Src/bInput/gcSave.cpp
    Producer: GNU C++ 2.95.3 SN BUILD v1.62 for Nintendo Gamecube
    Compile directory: b:\BlitzSDK\Babel\GameCube\Src\bInput
    Language: C++
    Code range: 0x80249024 -> 0x8024B758
    GCC Source File Name Offset: 0x0001B610
    GCC Source Info Offset: 0x0004E2D0
*/
...
// Range: 0x80249024 -> 0x80249100
int bInitGameSave() {
    // Local variables
    int c; // r31
}

// Range: 0x80249100 -> 0x80249140
void bShutdownGameSave() {}

// Range: 0x80249140 -> 0x80249378
int bGameSaveCheckError(int result /* r3 */) {}

// Range: 0x80249378 -> 0x80249668
void bGameSaveMainCallback(int chan /* r3 */, int result /* r4 */) {
    // Local variables
    int size; // r31
    struct _TBMemoryCardHeader * header; // r27
    unsigned int headerCrc;
    int headerSize;
}

// total size: 0x60
struct _TBMemoryCardHeader {
    // Members
    int version; // offset 0x0, size 0x4
    unsigned int dataSize; // offset 0x4, size 0x4
    unsigned int data_crc; // offset 0x8, size 0x4
    unsigned int header_crc; // offset 0xC, size 0x4
    unsigned short description[40]; // offset 0x10, size 0x50
};
// Range: 0x80249668 -> 0x8024973C
int verifyAndFixCardStatusInfo() {
    // Local variables
    int c; // r10
    int result;
}

// Range: 0x8024973C -> 0x80249940
static void bGameSaveCheckingMedia() {
    // Local variables
    char buf[256]; // r1+0x8
    int previousFlags; // r28
    int card; // r29
    int memSize; // r1+0x108
    int sectorSize; // r1+0x10C
    int result; // r3
    int busy; // r26
}

// Range: 0x80249940 -> 0x8024999C
void bGameSaveFormat() {
    // Local variables
    int result;
}

// Range: 0x8024999C -> 0x802499F0
static void bGameSaveDetachCallback(int chan /* r5 */, int result /* r4 */) {
    // Local variables
    char buf[256]; // r1+0x8
}

// Range: 0x802499F0 -> 0x80249D84
static int bGameSavePrepareHeader(char * GameName /* r30 */, char * comment /* r26 */, struct _TBTexture * banner /* r27 */, struct _TBTexture * icon /* r25 */, void * saveData /* r21 */, int saveDataSize /* r22 */) {
    // Local variables
    enum EBTextureFormat format; // r1+0x14
    unsigned char * iconData; // r3
    unsigned char * iconPalette; // r1+0x18
    unsigned char * bannerData; // r31
    unsigned char * bannerPalette; // r1+0x28
    int iconW; // r1+0x8
    int iconH; // r1+0xC
    int iconPitch; // r1+0x10
    int noofFrames; // r24
    int bannerW; // r1+0x1C
    int bannerH; // r1+0x20
    int bannerPitch; // r1+0x24
    unsigned char * src; // r31
    unsigned char * dst; // r28
    int y; // r30
    struct _TBMemoryCardHeader * header;
    int c; // r29
    int headerSize;
}
...
// Range: 0x80249D84 -> 0x8024A138
static void bGameSaveCommon(unsigned short * saveDataName /* r3 */, struct _TBGameSaveInfo * saveInfo /* r4 */, int deleteFile /* r5 */) {
    // Local variables
    int result;
...
struct _TBGameSaveInfo {
    ...
    struct _TBTexture * banner; // offset 0x40, size 0x4
    struct _TBTexture * icon; // offset 0x44, size 0x4
    unsigned char companyCode[2]; // offset 0x48, size 0x2
    unsigned char pad[2]; // offset 0x4A, size 0x2
};
// Range: 0x8024A138 -> 0x8024ABD0
static void bGameSaveReadingFileSystem(void * saveData /* r10 */, unsigned short * saveNamePrefix /* r27 */, int saveDataSize /* r28 */, struct _TBGameSaveDirEntry * dirEntries /* r31 */, int numDirEntries /* r7 */, int * numFiles /* r29 */, struct _TBGameSaveInfo * saveInfo /* r30 */) {
    // Local variables
    char buf[256]; // r1+0x8
    int result; // r3
    int byteNotUsed; // r1+0x130
    int filesNotUsed; // r1+0x134
    struct OSCalendarTime ct; // r1+0x108
    int size; // r5

    /* anonymous block */ {
        // Range: 0x8024A350 -> 0x8024A3E4
        int j; // ctr
        unsigned int id; // r7
    }

    /* anonymous block */ {
        // Range: 0x8024AA3C -> 0x8024AB0C
        int extraSize; // r11
    }
}
...
// Range: 0x8024ABD0 -> 0x8024B160
void bGameSaveSave(void * saveData /* r27 */, unsigned short * saveDataName /* r3 */, int saveDataSize /* r31 */, struct _TBGameSaveDirEntry * dirEntries /* r6 */, int numDirEntries /* r7 */, int * numFiles /* r8 */, int deleteFile /* r5 */, struct _TBGameSaveInfo * saveInfo /* r4 */) {
    // Local variables
    char buf[256]; // r1+0x8
    int result;
    int size; // r29
...
// Range: 0x8024B160 -> 0x8024B450
void bGameSaveLoad(void * saveData /* r30 */, unsigned short * saveDataName /* r3 */, int bufferSize /* r31 */, struct _TBGameSaveDirEntry * dirEntries /* r6 */, int numDirEntries /* r7 */, int * numFiles /* r8 */, struct _TBGameSaveInfo * saveInfo /* r4 */) {
    // Local variables
    char buf[256]; // r1+0x8
    int result; // r3
}

// Range: 0x8024B450 -> 0x8024B4D8
void bGameSaveDelete(void * saveData /* r3 */, unsigned short * saveDataName /* r3 */, int saveDataSize /* r5 */, struct _TBGameSaveDirEntry * dirEntries /* r6 */, int numDirEntries /* r7 */, int * numFiles /* r8 */, int deleteFile /* r5 */, struct _TBGameSaveInfo * saveInfo /* r4 */) {}

// Range: 0x8024B4D8 -> 0x8024B628
void bGameSavePoll(void * saveDataPtr /* r3 */, unsigned short * saveNamePrefix /* r11 */, unsigned short * saveDataName /* r4 */, int saveDataSize /* r5 */, struct _TBGameSaveDirEntry * dirEntries /* r6 */, int numDirEntries /* r7 */, int * numFiles /* r8 */, struct _TBGameSaveInfo * saveInfo /* r10 */, int deleteFile /* r9 */) {}

// Range: 0x8024B628 -> 0x8024B63C
void bGameSaveReset() {}
...
// Range: 0xFFFFFFFF -> 0x8024B63C
enum EBGameSaveSlotStatus biGameSaveGetCardSlotStatus(int slot /* r8 */, long * size /* r4 */, long * free /* r5 */) {}
...
// Range: 0x8024B63C -> 0x8024B698
int biGameSaveSetActiveCardSlot(int slot /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8024B698
int biGameSaveGetActiveCardSlot() {}

// Range: 0x8024B698 -> 0x8024B6CC
int bGameSaveActive() {}

// Range: 0x8024B6CC -> 0x8024B758
int biGameSaveCheckExistence(unsigned short * name /* r30 */, unsigned int productID /* r4 */) {
    // Local variables
    int i;
    unsigned short saveName16[64]; // r1+0x8
    int length; // r29
}
...
enum EBGameSaveMode bCurrentGameSaveMode; // size: 0x4
...
// total size: 0x7C
struct _TBMemoryCardInfo {
    // Members
    int flags; // offset 0x0, size 0x4
    int sectorSize; // offset 0x4, size 0x4
    int memorySize; // offset 0x8, size 0x4
    int memoryFree; // offset 0xC, size 0x4
    int filesFree; // offset 0x10, size 0x4
    int bufferSize; // offset 0x14, size 0x4
    void * buffer; // offset 0x18, size 0x4
    int lastBytesTransfered; // offset 0x1C, size 0x4
    int fileSize; // offset 0x20, size 0x4
    int fileOffset; // offset 0x24, size 0x4
    int bufferOffset; // offset 0x28, size 0x4
    int version; // offset 0x2C, size 0x4
    unsigned short description[32]; // offset 0x30, size 0x40
    int dataSize; // offset 0x70, size 0x4
    unsigned int header_crc; // offset 0x74, size 0x4
    unsigned int data_crc; // offset 0x78, size 0x4
};
typedef struct _TBMemoryCardInfo TBMemoryCardInfo;
typedef struct _TBMemoryCardHeader TBMemoryCardHeader;
enum EBGameSaveSubPhase {
    SUBPHASE_PREPARE = 0,
    SUBPHASE_MOUNT = 1,
    SUBPHASE_CHECK = 2,
    SUBPHASE_FREE = 3,
    ...
    SUBPHASE_OPEN = 6,
    SUBPHASE_CREATE = 7,
    SUBPHASE_READ_HEADER = 8,
    SUBPHASE_READ = 9,
    SUBPHASE_WRITE_HEADER = 10,
    SUBPHASE_WRITE = 11,
    SUBPHASE_GETSTATUSINFO = 12,
    SUBPHASE_SETSTATUSINFO = 13,
    SUBPHASE_CLOSE = 14,
    SUBPHASE_UMOUNT_DONE = 15,
    SUBPHASE_UMOUNT_FAIL = 16,
    SUBPHASE_CORRUPT_EXISTING_FILE_OPEN = 17,
    SUBPHASE_CORRUPT_EXISTING_FILE_WRITE = 18,
};
static int cardSlot; // size: 0x4, address: 0x8047D674
static TBMemoryCardInfo cardInfo[2]; // size: 0xF8, address: 0x8042B278
static unsigned char * cardWorkArea; // size: 0x4, address: 0x8047D678
static CARDFileInfo cardFileInfo; // size: 0x14, address: 0x8042B370
static CARDStat cardStatusInfo; // size: 0x6C, address: 0x8042B384
static unsigned char * cardBuffer; // size: 0x4, address: 0x8047D67C
static unsigned char * cardCorruptBuffer; // size: 0x4, address: 0x8047D680
static int cardHeaderSize; // size: 0x4, address: 0x8047D684
static int cardHeaderBanner; // size: 0x4, address: 0x8047D688
static int cardHeaderIcons; // size: 0x4, address: 0x8047D68C
static int cardFileLUT[64]; // size: 0x100, address: 0x8042B3F0
static enum EBGameSaveSubPhase subPhase; // size: 0x4, address: 0x8047D690
static enum EBGameSaveSubPhase subPhaseOnPass; // size: 0x4, address: 0x8047D694
static enum EBGameSaveStatus statusOnFail; // size: 0x4, address: 0x8047D698
static enum EBGameSaveStatus statusOnPass; // size: 0x4, address: 0x8047D69C
static enum EBGameSaveSubPhase subPhaseInProgress; // size: 0x4, address: 0x8047D6A0
static volatile int functionInProgress; // size: 0x4, address: 0x8047D6A4
static int formatInProgress; // size: 0x4, address: 0x8047D6A8
static int fileAt; // size: 0x4, address: 0x8047D6AC
static int filesFound; // size: 0x4, address: 0x8047D6B0
static int createFile; // size: 0x4, address: 0x8047D6B4
static char bDetectFileEntry[128][16]; // size: 0x800, address: 0x8042B4F0
static int bDetectGetDirCount; // size: 0x4, address: 0x8047CEF8
int bGameSaveExtraSize; // size: 0x4, address: 0x8047CEFC
int bGameSaveFlags; // size: 0x4
