/*
    Compile unit: b:/BlitzSDK/Babel/Common/Src/bInput/input.cpp
    Producer: GNU C++ 2.95.3 SN BUILD v1.62 for Nintendo Gamecube
    Compile directory: b:\BlitzSDK\Babel\GameCube\Src\bInput
    Language: C++
    Code range: 0x8024B758 -> 0x8024D588
    GCC Source File Name Offset: 0x0001B668
    GCC Source Info Offset: 0x0004E2F4
*/
...
// Range: 0x8024B758 -> 0x8024B7F4
void bInitInput(unsigned int flags /* r30 */) {}

// Range: 0x8024B7F4 -> 0x8024B870
void bShutdownInput() {}
...
// Range: 0x8024B870 -> 0x8024B948
struct _TBIChannelMap * biCreateMap(int maxChannels /* r29 */) {
    // Local variables
    struct _TBIChannelMap * map; // r31
}
...
// Range: 0x8024B948 -> 0x8024B9A4
void biDeleteMap(struct _TBIChannelMap * map /* r3 */) {}

// Range: 0x8024B9A4 -> 0x8024B9D8
void biDeleteAllMaps() {}

// Range: 0x8024B9D8 -> 0x8024BB40
struct _TBIInputChannel * biCreateChannel(struct _TBIChannelMap * map /* r28 */, char * name /* r4 */, int type /* r27 */, unsigned int flags /* r30 */, int maxValue /* r29 */, int accIncSpeed /* r26 */, int accDecSpeed /* r25 */) {
    // Local variables
    struct _TBIInputChannel * channel; // r31
    int l; // r11
}

// Range: 0xFFFFFFFF -> 0x8024BB40
static int InputIDValidOnPlatfrom(int inputID /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8024BB40
int biGetChannelName(struct _TBIChannelMap * map /* r3 */, int channelNumber /* r4 */, unsigned short * name /* r5 */, int * posInputID /* r28 */, int * negInputID /* r26 */) {
    // Local variables
    struct _TBIInputChannel * channel; // r27
    int c; // r29
}

// Range: 0xFFFFFFFF -> 0x8024BB40
void biDeleteChannel(struct _TBIInputChannel * channel /* r3 */) {}

// Range: 0x8024BB40 -> 0x8024BC7C
int biBindInput(struct _TBIInputChannel * channel /* r31 */, int posInputID /* r7 */, int negInputID /* r30 */) {
    // Local variables
    int val1; // r1+0x8
    int val2; // r1+0xC
    int l; // r30
...
// Range: 0xFFFFFFFF -> 0x8024BC7C
int biBindMulti(struct _TBIInputChannel * channel /* r30 */, int noofInputs /* r28 */, ...) {
    // Local variables
    ...
    /* anonymous block */ {
        // Range: 0x8024BC7C -> 0x8024BC7C
        int * __ptr; // r9
    }
}

// Range: 0xFFFFFFFF -> 0x8024BC7C
void biResetChannel(struct _TBIInputChannel * channel /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8024BC7C
void biResetMap(struct _TBIChannelMap * map /* r9 */) {}

// Range: 0x8024BC7C -> 0x8024BD00
void biEnableMap(struct _TBIChannelMap * map /* r3 */, int newState /* r4 */) {
    // Local variables
    int l; // r10
    struct _TBIInputChannel * channel; // r9
}

// Range: 0xFFFFFFFF -> 0x8024BD00
void biZeroMap(struct _TBIChannelMap * map /* r3 */) {
    // Local variables
    int l; // r11
}

// Range: 0x8024BD00 -> 0x8024BD2C
void biForceDebounce(struct _TBIInputChannel * channelPtr /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8024BD2C
void biDisableChannel(struct _TBIInputChannel * channelPtr /* r3 */, int disableTime /* r4 */) {}

// Range: 0x8024BD2C -> 0x8024BE54
static int bReadInputValue(struct _TBIInputChannel * channel /* r29 */, int inputID /* r31 */, int behaviour /* r28 */, int * valuePtr /* r30 */) {
    // Local variables
    int devID; // r11
    int gain; // r10
    int ret; // r3
...
// Range: 0x8024BE54 -> 0x8024BF64
static int bKeyValidForPlatform(int key /* r3 */, int flags /* r4 */) {
    // Local variables
    int valid; // r11
}

// Range: 0x8024BF64 -> 0x8024C2D0
static void bRandomMonkeyInput() {
    // Local variables
    int f; // r31
    int key; // r31
    unsigned long long time; // r24
    float press; // f1
    float release; // f31

    // Range: 0x8024C0D0 -> 0x8024C0D0
    inline float bmVanillaRandf() {
        // Range: 0x8024C0D0 -> 0x8024C0D0
        inline int bmVanillaRand() {}
    }

    // Range: 0x8024C118 -> 0x8024C118
    inline float bmVanillaRandf() {
        // Range: 0x8024C118 -> 0x8024C118
        inline int bmVanillaRand() {}
    }

    // Range: 0x8024C150 -> 0x8024C150
    inline int bmVanillaRand() {}

    // Range: 0x8024C1F4 -> 0x8024C1F4
    inline int bmVanillaRand() {}
}

// Range: 0xFFFFFFFF -> 0x8024C2D0
void biRandomMonkey(int active /* r30 */, int flags /* r29 */, int fingers /* r31 */, int devices /* r28 */, float minPressTime /* f28 */, float maxPressTime /* f29 */, float minReleaseTime /* f30 */, float maxReleaseTime /* f31 */) {
    // Local variables
    int c; // ctr

    /* anonymous block */ {
        // Range: 0x8024C2D0 -> 0x8024C2D0
        unsigned long long time; // r3
    }
}

// Range: 0x8024C2D0 -> 0x8024CBC8
int biReadDevices(float deltaT /* f31 */) {
    // Local variables
    struct _TBIChannelMap * mapPtr; // r20
    int l; // r3
    int k; // r23
    int i; // r29
    int ret; // r27
    int value; // r1+0x8
    int posValue; // r1+0xC
    int negValue; // r1+0x10
    int found; // r26
    int set; // r21
    struct _TBIInputChannel * channelPtr; // r31
    unsigned long long waitTime; // r3
    unsigned long long curTime; // r29
}

// Range: 0xFFFFFFFF -> 0x8024CBC8
int biGetChar() {}
...
// Range: 0xFFFFFFFF -> 0x8024CBC8
struct _TBInputRecording * biStartRecording(int clientContextSize /* r26 */) {
    // Local variables
    struct _TBIChannelMap * map; // r30
    int l; // r31
    int noofChannels; // r28
    int sampleSize; // r31
    struct _TBInputRecording * header; // r27
    struct _TBInputRecordingChannel * channelInfo; // r29
}
...
struct _TBInputRecordingChannel {
    // Members
    unsigned int crc; // offset 0x0, size 0x4
    struct _TBIInputChannel * channel; // offset 0x4, size 0x4
};
// Range: 0xFFFFFFFF -> 0x8024CBC8
struct _TBInputRecording * biRecordInputs(struct _TBInputRecording * recInfo /* r31 */, void * clientContext /* r29 */) {
    // Local variables
    void * newBuffer; // r30
    int l; // r8
    short * valPtr; // r3
    struct _TBInputRecordingChannel * channelInfo;
}

// Range: 0xFFFFFFFF -> 0x8024CBC8
int biStopRecording(struct _TBInputRecording * recInfo /* r3 */) {}

// Range: 0xFFFFFFFF -> 0x8024CBC8
void biStartPlayback(struct _TBInputRecording * recInfo /* r23 */) {
    // Local variables
    int l; // r11
    int k; // r27
    struct _TBIChannelMap * map; // r30
    struct _TBInputRecordingChannel * channelInfo; // r26
}

// Range: 0xFFFFFFFF -> 0x8024CBC8
int biPlaybackInputs(struct _TBInputRecording * recInfo /* r31 */, void * contextBuffer /* r3 */) {
    // Local variables
    int l; // r10
    struct _TBInputRecordingChannel * channelInfo;
    short * valPtr; // r4
...
// Range: 0xFFFFFFFF -> 0x8024CBC8
void * bEndianSwapPtr(void * ptr /* r3 */) {
    // Local variables
    unsigned int i; // r1+0x8
    unsigned int temp; // r1+0xC
    unsigned char * byteIn; // r7
    unsigned char * byteOut; // r10
    int c; // r8
    int n;
}

// Range: 0x8024CBC8 -> 0x8024CC00
int bEndianSwapInt(int i /* r1+0x8 */) {
    // Local variables
    unsigned int temp; // r11
    unsigned int c0; // r11
    unsigned int c1; // r0
    unsigned int c2; // r9
    unsigned int c3; // r3
...
// Range: 0xFFFFFFFF -> 0x8024CC00
short bEndianSwapShort(short s /* r1+0x8 */) {
    // Local variables
    unsigned short temp;
    unsigned short c0;
    unsigned short c1;
...
// Range: 0xFFFFFFFF -> 0x8024CC00
void bFixupPlayBackData(struct _TBInputRecording * recInfo /* r30 */) {
    // Local variables
    int l; // r29
    int s; // r9
    struct _TBInputRecordingChannel * channelInfo; // r9
    short * valPtr; // r31
}

// Range: 0xFFFFFFFF -> 0x8024CC00
void biSetMapPhysicalPort(struct _TBIChannelMap * map /* r30 */, int physicalPortId /* r28 */) {
    // Local variables
    int l; // r31
}

// Range: 0xFFFFFFFF -> 0x8024CC00
void biSetChannelPhysicalPort(struct _TBIInputChannel * channel /* r3 */, int physicalPortId /* r4 */) {
    // Local variables
    int l; // r11
    int k;
}
...
// Range: 0xFFFFFFFF -> 0x8024CC00
void biSwapMapPhysicalPort(struct _TBIChannelMap * map /* r30 */, int oldPhysicalPortId /* r27 */, int newPhysicalPortId /* r28 */) {
    // Local variables
    int l; // r31
}

// Range: 0xFFFFFFFF -> 0x8024CC00
void biSwapChannelPhysicalPort(struct _TBIInputChannel * channel /* r3 */, int oldPhysicalPortId /* r4 */, int newPhysicalPortId /* r5 */) {
    // Local variables
    int l; // r9
    int k;
}
...
// Range: 0xFFFFFFFF -> 0x8024CC00
int biAddRumbleEffect(int padId /* r3 */, int motorId /* r4 */, int amplitude /* r12 */, int duration /* r23 */, int attack /* r27 */, int sustain /* r22 */, int delay /* r25 */, enum EBRumbleStyle style /* r21 */) {
    // Local variables
    struct _TBRumbleController * ctrl; // r28
    struct _TBRumbleControllerEffect * effect; // r31
    int l; // r9
}
...
struct _TBRumbleController {
    // Members
    struct _TBRumbleControllerEffect effects[8]; // offset 0x0, size 0x140
...
// Range: 0xFFFFFFFF -> 0x8024CC00
void biAddRumbleBatchEffect(int padId /* r29 */, int noofEffects /* ctr */, struct _TBRumbleBatchEffect * effects /* r31 */) {
    // Local variables
    int l; // r30
}
...
// Range: 0x8024CC00 -> 0x8024CC68
void biStopRumble(int padId /* r31 */) {}

// Range: 0x8024CC68 -> 0x8024D500
void bProcessRumble() {
    // Local variables
    int pad; // r18
    int motor; // r11
    ...
    int delta; // r10
    int amp; // r26
    struct _TBRumbleController * ctrl; // r23
    struct _TBRumbleControllerEffect * effect; // r27
    unsigned long long curTime; // r28
    unsigned long long timeDelta; // r30
    float phase; // f0
    unsigned long long lastTime; // @ 0x8047CF20
    int first; // @ 0x8047CF28

    // Range: 0x8024D07C -> 0x8024D0E4
    inline float bmVanillaFMod(float x, float y) {
        // Local variables
        float v; // f0
        float py;
    ...
    // Range: 0x8024D0E4 -> 0x8024D0E4
    inline float bmVanillaFAbs(float x) {}

    // Range: 0x8024D208 -> 0x8024D270
    inline float bmVanillaFMod(float x, float y) {
        // Local variables
        float v; // f0
        float py;
    ...
    // Range: 0x8024D434 -> 0x8024D434
    inline int bmVanillaRand() {}
}

// Range: 0xFFFFFFFF -> 0x8024D500
void biSetAutoRepeat(struct _TBIInputChannel * channel /* r26 */, int startDelay /* r28 */, int repeatsPerSecond /* r27 */) {}

// Range: 0x8024D500 -> 0x8024D588
void biSetAutoRepeatMS(struct _TBIInputChannel * channel /* r26 */, int startDelay /* r28 */, int repeatDelay /* r27 */) {}
...
TBInputStateInfo bInputState; // size: 0x4, address: 0x8047D6B8
TBRumbleController bRumbleControllers[1][4]; // size: 0x520, address: 0x8042BCF0
TBIChannelMap * bInputMapList; // size: 0x4, address: 0x8047CF04
enum EBGameSaveMode {
    BGAMESAVE_LOAD = 0,
    BGAMESAVE_SAVE = 1,
    BGAMESAVE_DELETE = 2,
...
// total size: 0x28
struct _TBRandomMonkeyFinger {
    // Members
    unsigned long long pressDuration; // offset 0x0, size 0x8
    unsigned long long releaseDuration; // offset 0x8, size 0x8
    unsigned long long timer; // offset 0x10, size 0x8
    int pressing; // offset 0x18, size 0x4
    int key; // offset 0x1C, size 0x4
    int hitValue; // offset 0x20, size 0x4
};
typedef struct _TBRandomMonkeyFinger TBRandomMonkeyFinger;
// total size: 0x580
struct _TBRandomMonkey {
    // Members
    float minPressTime; // offset 0x0, size 0x4
    float variablePressTime; // offset 0x4, size 0x4
    float minReleaseTime; // offset 0x8, size 0x4
    float variableReleaseTime; // offset 0xC, size 0x4
    int flags; // offset 0x10, size 0x4
    int fingers; // offset 0x14, size 0x4
    int devices; // offset 0x18, size 0x4
    int keyValue[344]; // offset 0x1C, size 0x560
    TBRandomMonkeyFinger * finger; // offset 0x57C, size 0x4
};
typedef struct _TBRandomMonkey TBRandomMonkey;
TBIChannelMap * bExclusiveMap; // size: 0x4, address: 0x8047CF00
static int rumblePaused; // size: 0x4, address: 0x8047CF08
static unsigned long long rumbleTimer; // size: 0x8, address: 0x8047CF10
static TBRandomMonkey * bRandomMonkey; // size: 0x4, address: 0x8047CF18
